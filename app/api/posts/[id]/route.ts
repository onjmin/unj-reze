...
export async function PATCH(
  ...
  const { userId, content, originType, imageSrc, checkeredDark } = await request.json();
  if (!userId || typeof content !== 'string') {
    return NextResponse.json({ error: 'userId and content are required' }, { status: 400 });
  }
  const result = await db.editPost(decodedId, userId, content, originType, imageSrc, checkeredDark);
  if (!result) {
