...
import { ArrowLeft, Loader2, MoreHorizontal, Send, Trash2, Users } from 'lucide-react';
...
import { canSendDm, isDmOpen, FIRST_DM_NOTICE, type DmGate } from '@/lib/dm-rules';
...
export default function DmThreadView({ partnerSlug }: DmThreadViewProps) {
  ...
  useEffect(() => {
    if (!myId) return;
    Promise.all([
      api.follow.isFollowing(myId, partnerSlug).then(r => r.isFollowing).catch(() => false),
      api.follow.isFollowing(partnerSlug, myId).then(r => r.isFollowing).catch(() => false),
    ]).then(([iFollow, followsMe]) => {
      setRelation(iFollow && followsMe ? 'mutual' : iFollow ? 'following' : followsMe ? 'followed' : 'none');
    });
  }, [myId, partnerSlug]);
  ...
  useEffect(() => {
    ...
    api.messages.conversation(myId, partnerSlug)
      .then(res => {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const unsubHandler = client.addHandler(msg => {
      ...
      if (!myIdentifiers.has(data.sender)) setGate(g => ({ ...g, received: g.received + 1 }));
    ...
    <div className="flex flex-col flex-1 min-h-[calc(100vh-56px)] md:min-h-screen">
      <div className="sticky top-0 z-20 bg-[#0b0e14]/95 backdrop-blur border-b border-gray-800">
        <div className="flex items-center px-3 h-11 gap-2">
          ...
          >
            {partnerAvatar ? (
              <img src={partnerAvatar} alt={partnerInfo.username} className="w-full h-full object-cover" />
            ) : (
      ...
      <div className="flex-1 flex flex-col px-4 py-4 gap-3">
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center">
            ...
            <span className="font-bold text-sm text-gray-100">{partnerInfo.username}</span>
            {partnerFollowers !== null && (
              <span className="text-[11px] text-gray-400 flex items-center gap-1.5">
                <Users size={12} className="text-gray-500" />
                フォロワー <span className="font-bold text-gray-200">{partnerFollowers}</span>
              </span>
            )}
            <span className="text-[11px] text-gray-500">{RELATION_LABEL[relation]}</span>
            <button
              onClick={() => router.push(`/user/${partnerSlug}`)}
          ...
          <div className="flex-1 flex flex-col gap-3">
            {ordered.map(m => {
              const mine = isMine(m);
              return (
                <div key={m.id} className={`flex flex-col group w-full ${mine ? 'items-end' : 'items-start'}`}>
                  <span className="text-[10px] text-gray-500 mb-1 px-1">{m.time}</span>
                  <div className={`flex items-end gap-1.5 max-w-[85%] sm:max-w-[75%] ${mine ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div
        ...
        {!loading && !open && (
          <div className="mx-auto max-w-sm text-center text-[11px] leading-relaxed text-gray-400 bg-gray-100/5 border border-gray-800 rounded-xl px-3.5 py-2.5">
            {canSend ? FIRST_DM_NOTICE : '1通目を送信済みです。相手から返信があるまで続けて送ることはできません。'}
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      ...
      <div className="sticky bottom-14 md:bottom-0 z-20 p-3 border-t border-gray-800 bg-[#0b0e14]/95 backdrop-blur">
        {sendError && <div className="text-[11px] text-red-400 mb-2 px-1">{sendError}</div>}
        {!mounted ? (
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={canSend ? 'メッセージを入力...' : '相手からの返信を待っています'}
      ...
      {menuPos && (
        <UserActionMenu
          ...
          onMention={(username) => router.push(`/?mention=${encodeURIComponent(username)}`)}
          position={menuPos}
          hideDm
        />
      )}
