...
function parseInitialCrop(ref: string): InitialCrop | null {
  ...
  if (ref.startsWith('url:')) {
    ...
    const parts = ref.slice(hashIdx + 1).split(',').map(Number);
    if (parts.length < 4 || parts.slice(0, 4).some(n => isNaN(n))) return null;
    const [x, y, w, h] = parts;
...
function SliceEditor({ image, initialRef, onBack, onPick, confirmLabel }: {
  ...
  const [outKind, setOutKind] = useState<'sprite' | 'walk'>(initialCrop?.outKind ?? 'sprite');

  // 歩行グラの「コマ数/サイズ」。テンプレ選択時は既定値を一度だけ流し込むだけで、
  // 以後は自由に編集できる入力欄にする（選択に連動して固定はしない）。
  const [templateId, setTemplateId] = useState<string>(initialCrop?.templateId ?? WALK_STANDARDS[0].id);
  const [fieldW, setFieldW] = useState<number>(initialCrop?.fieldW ?? WALK_STANDARDS[0].w);
  ...
  const wayKeys = useMemo(
    () => fieldWays.toLowerCase().split('').filter((c): c is WayKey => c in WAY),
    [fieldWays],
  );

  // 描画時のセル下端揃え位置（下端からの距離px・既定0・負値可）と表示倍率（小数可・未指定はセルに自動フィット）。
  const [fieldOffsetY, setFieldOffsetY] = useState(initialCrop?.fieldOffsetY ?? 0);
  ...
  const isWalkTemplateReady = outKind === 'walk' && fieldW > 0 && fieldH > 0 && fieldFrames > 0 && wayKeys.length > 0;
  ...
  useEffect(() => {
    ...
    loadImage(url).then(im => {
      if (!alive) return;
      setImg(im);
      const s = Math.min(1, DISPLAY_MAX / Math.max(im.naturalWidth, im.naturalHeight));
      setScale(s);
  ...
  useEffect(() => {
    ...
    if (gridMode || isWalkTemplateReady) {
      ...
      for (let x = 0; x <= dw; x += blockW * scale) {
        ctx.beginPath(); ctx.moveTo(x + 0.5, 0); ctx.lineTo(x + 0.5, dh); ctx.stroke();
      }
      for (let y = 0; y <= dh; y += blockH * scale) {
        ctx.beginPath(); ctx.moveTo(0, y + 0.5); ctx.lineTo(dw, y + 0.5); ctx.stroke();
      }
    }
  ...
  }, [img, scale, rect, gridMode, tileW, tileH, isWalkTemplateReady, blockW, blockH]);
  ...
  useEffect(() => {
    ...
    if (!canvas || !img || outKind !== 'walk' || !rect || rect.w <= 0 || rect.h <= 0 || wayKeys.length === 0) return;
    const ctx = canvas.getContext('2d');
    if (!ctx) return;

    const std = { id: 'preview', label: '', w: rect.w / fieldFrames, h: rect.h / wayKeys.length, frames: fieldFrames, ways: wayKeys.map(k => WAY[k]) };
    ...
    const render = (t: DOMHighResTimeStamp) => {
      ...
      for (let y = 0; y < CH; y += check) {
        for (let x = 0; x < CW; x += check) {
          ctx.fillStyle = ((x / check + y / check) % 2 === 0) ? '#1a1d26' : '#12141a';
          ctx.fillRect(x, y, check, check);
        }
      }

      const userScale = parseFloat(fieldScale);
      const zoom = !isNaN(userScale) && userScale > 0 ? userScale : CELL / cell.sh;
      ...
      ctx.imageSmoothingEnabled = false;
      ctx.drawImage(img, cell.sx, cell.sy, cell.sw, cell.sh, destX, destY, destW, destH);

      // セル枠（想定タイル1マス分）
      ctx.strokeStyle = 'rgba(255,255,255,0.25)';
      ctx.lineWidth = 1;
      ctx.strokeRect((CW - CELL) / 2 + 0.5, FLOOR_Y - CELL + 0.5, CELL - 1, CELL - 1);

      // 床（セル下端）ライン
      ctx.strokeStyle = 'rgba(255,255,255,0.5)';
      ctx.setLineDash([]);
      ctx.beginPath(); ctx.moveTo(0, FLOOR_Y + 0.5); ctx.lineTo(CW, FLOOR_Y + 0.5); ctx.stroke();
      ...
      ctx.lineWidth = 2;
      ctx.setLineDash([4, 3]);
      ctx.beginPath(); ctx.moveTo(0, alignY + 0.5); ctx.lineTo(CW, alignY + 0.5); ctx.stroke();
      ctx.setLineDash([]);
    };
    raf = requestAnimationFrame(render);
  ...
  const confirm = () => {
    ...
    if (outKind === 'sprite') {
      ...
      onPick({ ref: `url:${url}#${crop}`, url, label: `画像 #${image.id} 切り出し` });
    } else {
      // 幅/高さ/コマ数/方向転換が既知の規格と完全一致すればその規格IDを使う。
      // 自由に編集済みでどれとも一致しなければ stdId=auto + コマ数指定（方向の並びは自動判定に委ねる）。
      const waysStr = wayKeys.join('');
      const matched = WALK_STANDARDS.find(s =>
        s.w === fieldW && s.h === fieldH && s.frames === fieldFrames && s.ways.map(w => w.key).join('') === waysStr);
  ...
  if (error) {
    return (
      <div className="flex flex-col gap-2">
        <button onClick={onBack} className="flex items-center gap-1 text-[10px] text-blue-500 hover:underline font-bold w-fit"><ArrowLeft size={11} />投稿一覧に戻る</button>
        <p className="text-center text-[11px] text-red-500 py-8">画像の読み込みに失敗しました。</p>
      </div>
    );
  }
        ...
        <>
          <div ref={wrapRef} className="flex justify-center bg-black/40 border border-gray-800 rounded-lg p-2 overflow-auto gimp-checkered-background">
            <canvas
              ...
              style={{ imageRendering: 'pixelated', touchAction: 'none', cursor: (gridMode || isWalkTemplateReady) ? 'pointer' : 'crosshair' }}
            />
          </div>
          ...
          {outKind === 'sprite' && (
            <>
              <div className="flex flex-wrap gap-1.5">
                <button className={btn(!gridMode)} onClick={() => setGridMode(false)}>✂️ 自由選択</button>
                <button className={btn(gridMode)} onClick={() => setGridMode(true)}>▦ グリッド分割</button>
              </div>

              {gridMode && (
                <div className="flex items-center gap-2 text-[10px] text-gray-500">
                  <span>マスサイズ(px):</span>
                  <input type="number" min={1} value={tileW} onChange={e => setTileW(Math.max(1, parseInt(e.target.value, 10) || 1))}
                    className="w-14 bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" />
                  <span>×</span>
                  <input type="number" min={1} value={tileH} onChange={e => setTileH(Math.max(1, parseInt(e.target.value, 10) || 1))}
                    className="w-14 bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" />
                  <span className="text-gray-600">（RPGツクール/RPGEN規格: 16, 32, 48等）</span>
                </div>
              )}
            </>
          )}
          ...
          {outKind === 'walk' && (
            <div className="flex flex-col gap-2 p-2.5 rounded-lg border border-gray-800 bg-[#11131a]">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <label className="flex flex-col gap-0.5 text-[10px] text-gray-500">
                  幅
                  <input type="number" min={1} value={fieldW} onChange={e => setFieldW(Math.max(1, parseInt(e.target.value, 10) || 1))}
                    className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" />
                </label>
                <label className="flex flex-col gap-0.5 text-[10px] text-gray-500">
                  高さ
                  <input type="number" min={1} value={fieldH} onChange={e => setFieldH(Math.max(1, parseInt(e.target.value, 10) || 1))}
                    className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" />
                </label>
              </div>
              ...
              <label className="flex flex-col gap-0.5 text-[10px] text-gray-500">
                コマ数
                <input type="number" min={1} value={fieldFrames} onChange={e => setFieldFrames(Math.max(1, parseInt(e.target.value, 10) || 1))}
                  className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" />
              </label>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <label className="flex flex-col gap-0.5 text-[10px] text-gray-500">
                  下端揃え位置(px)
                  <input type="number" step={1} value={fieldOffsetY} onChange={e => setFieldOffsetY(parseInt(e.target.value, 10) || 0)}
                    className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" />
                </label>
                <label className="flex flex-col gap-0.5 text-[10px] text-gray-500">
                  表示倍率
                  <input type="number" min={0} step={0.1} value={fieldScale} onChange={e => setFieldScale(e.target.value)} placeholder="自動"
                    className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" />
                </label>
              </div>
              <p className="text-[10px] text-gray-600">下端揃え位置: セル下端からの距離(px)。正で上、負で下にずれます。表示倍率: 小数も可（未指定はセルに自動フィット）。</p>

              {rect && rect.w > 0 && rect.h > 0 && wayKeys.length > 0 && (
                <div className="flex flex-col gap-1 items-center">
                  <span className="text-[10px] text-gray-500 self-start">揃え位置プレビュー（オレンジの下線が揃え位置）</span>
                  <canvas ref={alignCanvasRef} width={160} height={110} className="rounded-lg border border-gray-800" style={{ imageRendering: 'pixelated' }} />
                </div>
              )}
          ...
          <div className="grid grid-cols-4 gap-1.5 text-[10px] text-gray-500">
            <label className="flex flex-col gap-0.5">X<input type="number" value={rect?.x ?? 0} onChange={e => updateRectField('x', Number(e.target.value))} className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" /></label>
            <label className="flex flex-col gap-0.5">Y<input type="number" value={rect?.y ?? 0} onChange={e => updateRectField('y', Number(e.target.value))} className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" /></label>
            <label className="flex flex-col gap-0.5">幅<input type="number" value={rect?.w ?? 0} onChange={e => updateRectField('w', Number(e.target.value))} className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" /></label>
            <label className="flex flex-col gap-0.5">高さ<input type="number" value={rect?.h ?? 0} onChange={e => updateRectField('h', Number(e.target.value))} className="bg-gray-900 border border-gray-800 rounded px-1.5 py-1 text-gray-200" /></label>
          </div>
