...
import {
  searchSAnimSheets, searchSoundSheets, searchSpriteAnims, searchSounds,
  getSpriteAnimById, getSoundById,
  sAnimUrl, soundUrl,
...
const emptyCache = (): AssetCache => ({ query: '', submitted: '', sheets: [], items: [], page: 1, pages: 1, total: 0, open: null });
const caches: Record<Kind, AssetCache> = { walk: emptyCache(), sound: emptyCache() };
...
export default function RpgenAssetPanel({ kind, onPick }: RpgenAssetPanelProps) {
  ...
  const [sheets, setSheets] = useState<(SAnimSheetItem | SoundSheetItem)[]>(cache.sheets);
  const [items, setItems] = useState<(SpriteAnimItem | SoundItem)[]>(cache.items);
  const [page, setPage] = useState(cache.page);
  const [pages, setPages] = useState(cache.pages);
  const [total, setTotal] = useState(cache.total);
  const [loading, setLoading] = useState(cache.sheets.length === 0 && cache.items.length === 0);
  ...
  useEffect(() => {
    cache.query = query; cache.submitted = submitted; cache.sheets = sheets; cache.items = items;
    cache.page = page; cache.pages = pages; cache.total = total; cache.open = open;
  });
  ...
  useEffect(() => {
    ...
    Promise.all(ids.map((id) =>
      (kind === 'walk' ? getSpriteAnimById(id, ctrl.signal) : getSoundById(id, ctrl.signal)).then((res) => {
        const name = kind === 'walk' ? (res as SpriteAnimItem | null)?.name : (res as SoundItem | null)?.title;
        if (name) nameCache.set(id, name);
      }),
    )).then(() => { if (!ctrl.signal.aborted) bumpNames((v) => v + 1); });
    return () => ctrl.abort();
  }, [open, kind]);
  ...
  useEffect(() => {
    ...
    const req = submitted
      ? (kind === 'walk' ? searchSpriteAnims(params) : searchSounds(params)).then((res) => {
          setItems((prev) => (page === 1 ? res.data : [...prev, ...res.data]));
          setPages(res.meta.pages);
          setTotal(res.meta.total);
        })
      : (kind === 'walk' ? searchSAnimSheets(params) : searchSoundSheets(params)).then((res) => {
  ...
  const pickWalk = (m: SAnimSheetMember, sheetName: string) => {
    ...
    onPick({ ref: buildWalkRef('auto', { kind: 'url', url }), url, label: walkNameCache.get(m.id) || sheetName || `歩行グラ` });
  };

  const pickSound = (m: SoundSheetMember, sheetName: string) =>
    onPick({ ref: `direct:${soundUrl(m.id)}`, url: soundUrl(m.id), label: soundNameCache.get(m.id) || sheetName || 'SE' });

  const pickWalkItem = (item: SpriteAnimItem) => {
    const url = sAnimUrl(item.id);
    onPick({ ref: buildWalkRef('auto', { kind: 'url', url }), url, label: item.name || '歩行グラ' });
  };

  const pickSoundItem = (item: SoundItem) =>
    onPick({ ref: `direct:${soundUrl(item.id)}`, url: soundUrl(item.id), label: item.title || 'SE' });

  const searching = submitted !== '';
  const placeholder = kind === 'sound' ? '効果音を検索（例: 攻撃, ジャンプ）' : '歩行グラを検索（例: スライム, 兵士）';
              ...
              return (
                <div key={`${m.id}-${i}`} className="flex items-center gap-2 p-2 rounded-lg border border-gray-700 hover:border-blue-500 bg-gray-900">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-gray-200 font-bold truncate">{name ?? m.id}</p>
                    {name && <p className="text-[9px] text-gray-600 font-mono truncate">{m.id}</p>}
                  </div>
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-[10px] text-gray-600 px-0.5">
        {searching
          ? <>{kind === 'walk' ? '歩行グラ' : '効果音'}を検索{total > 0 && <> ・{total.toLocaleString()}件</>}</>
          ...
          {kind === 'walk' ? (
            ...
            <div className="space-y-1.5">
              {(items as SoundItem[]).map((item, i) => (
                <div key={`${item.id}-${i}`} className="flex items-center gap-2 p-2 rounded-lg border border-gray-700 hover:border-blue-500 bg-gray-900">
                  <button
