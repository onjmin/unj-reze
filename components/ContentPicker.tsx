...
import type { MmlPlayback, ParsedMML } from "@onjmin/dtm";
import { Loader2, Music, Play, Search, Square, Video, X } from "lucide-react";
...
const lastBgmTabByKind: Record<"bgm" | "sfx" | "mml", BgmTab> = {
	bgm: "youtube",
	sfx: "rpgenSe",
	mml: "mmlPost",
};
...
export default function ContentPicker({
	...
	const [bgmTab, setBgmTab] = useState<BgmTab>(() => {
		if (mode === "bgm" && currentRefBase) {
			if (
				currentRefBase.startsWith("youtube:") &&
				allowedBgmTabs.includes("youtube")
			)
				return "youtube";
			if (
				currentRefBase.startsWith("nicovideo:") &&
				allowedBgmTabs.includes("nicovideo")
			)
				return "nicovideo";
			if (
				currentRefBase.startsWith("soundcloud:") &&
				allowedBgmTabs.includes("soundcloud")
			)
				return "soundcloud";
			if (
				currentRefBase.startsWith("direct:") &&
				allowedBgmTabs.includes("direct")
			)
				return "direct";
			if (
				currentRefBase.startsWith("mml:post:") &&
				allowedBgmTabs.includes("mmlPost")
			)
				return "mmlPost";
			if (
				currentRefBase.startsWith("mml:") &&
	...
	const [urlInput, setUrlInput] = useState(() => {
		...
		if (currentRefBase.startsWith("youtube:"))
			return toYoutubeWatchUrl(currentRefBase.replace(/^youtube:/, ""));
		if (currentRefBase.startsWith("direct:"))
			return currentRefBase.replace(/^direct:/, "");
		return "";
	});
	...
	const [mmlStepInfo, setMmlStepInfo] = useState<{
		currentStep: number;
		totalSteps: number;
	} | null>(null);
	...
	const activeMmlCacheRef = useRef<{
		mml: string;
		parsed: ParsedMML;
		totalSteps: number;
		bpm: number;
	...
	const changeImageTab = (tab: ImageTab) => {
		stopAllPreviews();
	...
	const changeBgmTab = (tab: BgmTab) => {
		stopAllPreviews();
		Promise.resolve().then(() => {
			lastBgmTabByKind[bgmKind] = tab;
		});
		setBgmTab(tab);
	};
		...
		try {
			...
			const totalSteps =
				parsed.placements?.length > 0
					? Math.max(
							...parsed.placements.map((p) => p.startStep + p.durationSteps),
							192,
						)
					: 192;
			...
			if (startAt > 0 && dtm.playPlacements && parsed.placements) {
				const remaining = parsed.placements.filter(
					(p) => p.startStep + p.durationSteps > startAt,
				);
				const shifted = remaining.map((p) => ({
					...p,
					startStep: Math.max(0, p.startStep - startAt),
				}));
				bgm = dtm.playPlacements(shifted, {
					...
					onTick: (relStep: number) => {
						setMmlStepInfo({
							currentStep: Math.min(startAt + relStep, totalSteps),
							totalSteps,
						});
					},
					onStop: () => {
				...
				bgm = dtm.playMML(
					withMmlVolume(mml, effectiveMmlVolume(mml, applyMasterVolume(100))),
					{
						loop: false,
	...
	return (
		<div
			className="absolute inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/60"
			onClick={() => {
				stopAllPreviews();
				onClose();
			}}
			...
			>
				<div className="flex items-center justify-between px-4 py-3 border-b border-gray-800 shrink-0">
					...
					<button
						...
						className="text-gray-400 hover:text-gray-200 p-1 rounded-full hover:bg-gray-100/10"
					>
						<X size={16} />
					</button>
				</div>
				...
				<div className="flex flex-wrap gap-1 p-2 bg-[#0f0f11] border-b border-gray-800 shrink-0">
					{mode === "image" ? (
						<>
							...
							{usedAssets.length > 0 && (
								<button
									className={tabBtn(imageTab === "history")}
									onClick={() => changeImageTab("history")}
								>
									使用履歴
								</button>
							)}
							<button
								className={tabBtn(imageTab === "mySheet")}
								onClick={() => changeImageTab("mySheet")}
							>
								マイシート
							</button>
							<button
							...
							<button
								className={tabBtn(imageTab === "color")}
								onClick={() => changeImageTab("color")}
							>
								単色カラー
							</button>
							...
							<button
								...
								onClick={() => changeImageTab("local")}
							>
								内蔵素材
							</button>
							<button
								className={tabBtn(imageTab === "rpgenSprite")}
								onClick={() => changeImageTab("rpgenSprite")}
							>
								外部素材
							</button>
							<button
								className={tabBtn(imageTab === "rpgenWalk")}
								onClick={() => changeImageTab("rpgenWalk")}
							>
								外部歩行グラ
							</button>
							<button
						...
						<>
							{allowedBgmTabs.includes("youtube") && (
								<button
									className={tabBtn(bgmTab === "youtube")}
									onClick={() => changeBgmTab("youtube")}
								>
									<Video size={12} />
									YouTube
								</button>
							...
							{allowedBgmTabs.includes("nicovideo") && (
								<button
									className={tabBtn(bgmTab === "nicovideo")}
									onClick={() => changeBgmTab("nicovideo")}
								>
									ニコニコ
								</button>
							)}
							{allowedBgmTabs.includes("soundcloud") && (
								<button
									className={tabBtn(bgmTab === "soundcloud")}
									onClick={() => changeBgmTab("soundcloud")}
								>
									SoundCloud
								</button>
							)}
							{allowedBgmTabs.includes("mmlPost") && (
								<button
									className={tabBtn(bgmTab === "mmlPost")}
									onClick={() => changeBgmTab("mmlPost")}
								>
									<Music size={12} />
									MML投稿
								</button>
							...
							{allowedBgmTabs.includes("rpgenSe") && (
								<button
									className={tabBtn(bgmTab === "rpgenSe")}
									onClick={() => changeBgmTab("rpgenSe")}
								>
									効果音
								</button>
							)}
							...
							{allowedBgmTabs.includes("mmlRaw") && (
								<button
									className={tabBtn(bgmTab === "mmlRaw")}
									onClick={() => changeBgmTab("mmlRaw")}
								>
									直接
								</button>
							)}
							{allowedBgmTabs.includes("direct") && (
								<button
									className={tabBtn(bgmTab === "direct")}
									onClick={() => changeBgmTab("direct")}
								>
									URL
								</button>
							)}
				...
				>
					...
					{mode === "image" && imageTab === "history" && (
						<div className="grid grid-cols-6 gap-2">
								...
								.map((a, i) => (
									...
									>
										<AssetThumb
											...
											url={a.url}
											onError={() =>
												setFailedRefs((prev) => new Set(prev).add(a.ref))
											}
										/>
							...
							{usedAssets.filter((a) => !failedRefs.has(a.ref)).length ===
								0 && (
								<p className="col-span-6 text-center text-[11px] text-gray-600 py-8">
									このゲームではまだ画像が使われていません
								</p>
							)}
						</div>
					)}
						...
						<div className="space-y-3">
							...
							{loading ? (
								...
								<div className="grid grid-cols-3 gap-2">
									...
									{imagePosts.filter((p) => !failedPostIds.has(p.id)).length ===
										0 && (
										<p className="col-span-3 text-center text-xs text-gray-500 py-8">
											画像投稿が見つかりませんでした
										</p>
									)}
								</div>
					...
					{mode === "bgm" && bgmTab === "nicovideo" && (
						<div className="space-y-2">
							...
							/>
							<p className="text-[10px] text-gray-600">
								ニコニコ動画をBGMとしてループ再生します。
							</p>
							<button
								onClick={pickNicovideo}
								disabled={!urlInput.trim()}
								className="w-full py-2 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-xs font-bold"
							>
								このBGMを使う
					...
					{mode === "bgm" && bgmTab === "soundcloud" && (
						<div className="space-y-2">
							...
							/>
							<p className="text-[10px] text-gray-600">
								SoundCloud 楽曲をBGMとしてループ再生します。
							</p>
							<button
								onClick={pickSoundCloud}
								disabled={!urlInput.trim()}
								className="w-full py-2 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-xs font-bold"
							>
								このBGMを使う
					...
					{mode === "bgm" && bgmTab === "mmlPost" && (
						<>
							<p className="text-[10px] text-[#a3e635] mb-2">
								♪
								MMLは楽譜データだけの超軽量BGM。音声ファイルを読み込まないので読み込みが一瞬で終わります。
							</p>
								...
								<div className="space-y-1.5">
									{mmlPosts.map((p) => {
										...
										const currentStep = isPrev
											...
											: 0;
										const totalSteps = isPrev
											? (mmlStepInfo?.totalSteps ?? 1)
											: 1;
										return (
											<div
												key={p.id}
												className="flex flex-col gap-1.5 p-2 rounded-lg border border-gray-700 hover:border-blue-500 bg-gray-900"
											>
												<div className="flex items-center gap-1.5">
													<button
														onClick={async () => {
															if (isPrev) {
																previewMml(key, inlineMml);
																return;
															}
															const mml = await resolveMmlText(p);
														...
														<input
															...
															onChange={(e) =>
																seekMml(
																	...
																	inlineMml ||
																		(p.mmlUrl
																			? mmlTextCacheRef.current.get(p.mmlUrl)
																			: undefined) ||
																		"",
					...
					{mode === "bgm" && bgmTab === "mmlRaw" && (
						<div className="space-y-2">
							...
							<div className="flex gap-2">
								<button
									onClick={() => previewMml("raw", mmlInput)}
									disabled={!mmlInput.trim()}
								...
								<button
									onClick={pickMmlRaw}
									disabled={!mmlInput.trim()}
									className="flex-1 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-xs font-bold"
								>
									このMMLを使う
								</button>
							</div>
							{previewKey === "raw" && mmlStepInfo && (
								<div className="flex items-center gap-2 pt-1">
									<input
										...
										value={Math.min(
											mmlStepInfo.currentStep,
											mmlStepInfo.totalSteps,
										)}
										onChange={(e) =>
											seekMml("raw", mmlInput, Number(e.target.value))
										}
										className="flex-1 h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-[#a3e635]"
									/>
									<span className="text-[9px] text-gray-400 font-mono">
										{Math.floor(
											(mmlStepInfo.currentStep / mmlStepInfo.totalSteps) * 100,
										)}
										%
									</span>
								</div>
					...
					{mode === "bgm" && bgmTab === "direct" && (
						<div className="space-y-2">
							...
							/>
							<p className="text-[10px] text-gray-600">
								MP3/WAV の直リンクURLを入力。効果音に向いています。
							</p>
							{urlInput.trim() && (
								<div className="flex items-center gap-2 p-2 rounded-lg bg-gray-900 border border-gray-700">
									...
									<div className="flex-1 min-w-0 flex flex-col gap-1">
										<div className="flex justify-between items-center text-[10px] text-gray-400">
											...
											{directDuration > 0 && (
												<span>
													{formatTime(directCurrentTime)} /{" "}
													{formatTime(directDuration)}
												</span>
											)}
										</div>
										{previewKey === "directUrl" && (
					...
					{mode === "bgm" && bgmTab === "rpgenSe" && (
						<RpgenAssetPanel
							...
							onPlayPreview={handleSubpanelPlayPreview}
						/>
					)}
