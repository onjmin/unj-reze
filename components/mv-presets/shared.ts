 ...
 */
export function rozeLoop(frames: number, cell: number, loopBeats = 4): MvWalkSetting {
  return { stdId: 'row_anim', crop: [0, 0, cell * frames, cell], frames, loopBeats, playMode: 'loop' };
}

/** beat-* 用（1コマ384px）。 */
export function rozeBeat(key: keyof typeof ROZE_BEATS, loopBeats = 4): MvWalkSetting {
  return rozeLoop(ROZE_BEATS[key].frames, 384, loopBeats);
}

/** pose-* 用（1コマ320px）。 */
export function rozePose(key: keyof typeof ROZE_POSES, loopBeats = 4): MvWalkSetting {
...
export function rozeSheetRow(row: number, loopBeats = 4): MvWalkSetting {
  return { stdId: 'row_anim', crop: [0, row * 64, 256, 64], frames: 4, loopBeats, playMode: 'loop' };
}
...
export const COOKIE_CAST = {
  'mgr-a': 4, 'mgr-b': 2, 'mgr-c': 2,
  'mot-a': 4, 'mot-b': 4, 'mot-c': 4,
  'nyn-a': 3, 'nyn-b': 3, 'nyn-c': 3, 'nyn-d': 3,
} as const;
...
export function cookieWalk(key: CookieKey, loopBeats = 4): MvWalkSetting {
  ...
  return { stdId: 'row_anim', crop: [0, 0, 160 * frames, 240], frames, loopBeats, playMode: 'loop' };
}

/** 夜の海辺（4コマ・256×192）。背景として全画面に敷く。 */
export const BEACH_NIGHT = { url: '/assets/mv/beach-night.png', frames: 4, w: 256, h: 192 };

export function beachWalk(loopBeats = 8): MvWalkSetting {
  return { stdId: 'row_anim', crop: [0, 0, BEACH_NIGHT.w * BEACH_NIGHT.frames, BEACH_NIGHT.h], frames: BEACH_NIGHT.frames, loopBeats, playMode: 'loop' };
}
