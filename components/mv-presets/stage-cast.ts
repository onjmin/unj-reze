...
const MML = [
	...
	mvTrack(
		"@1 t118 q60 v78 o2 l2",
		PROGRESSION.map((c) => `${CHORD_ROOT[c]} ${CHORD_ROOT[c]}`),
		BARS,
	),
	mvTrack(
		"@2 t118 q50 v52 o3 l2",
		PROGRESSION.map((c) => CHORD_MML[c]),
		BARS,
	),
...
const CHORD_STEPS: MvChordStep[] = PROGRESSION.map((label, bar) => ({
	bar,
	label,
}));
...
const SECTIONS: MvSection[] = [
	{
		...
		label: "A（3人）",
		startBar: 0,
		stage: { bgColor: "#9db3c9", bgDim: 0.02 },
	},
	{
		id: "a2",
		label: "A′（4人目）",
	...
	},
	{
		...
		label: "B（5人目）",
		startBar: 16,
		stage: { bgColor: "#96a9bf", bgDim: 0.05 },
		transition: { style: "fade", beats: 1 },
	},
	{
		id: "sabi",
		label: "サビ（6人目）",
		startBar: 24,
		stage: { bgColor: "#b0a2b0", bgDim: 0 },
		transition: { style: "flash", beats: 1 },
	},
	{
		id: "inter",
		label: "間奏",
		startBar: 32,
		stage: { bgColor: "#8b93ab", bgDim: 0.16 },
	...
	},
	{
		...
		label: "A″（7人目）",
		startBar: 40,
		stage: { bgColor: "#a8a0ae", bgDim: 0.08 },
		transition: { style: "fade", beats: 1 },
	},
	{
		id: "sabi2",
	...
	},
	{
		...
		label: "アウトロ（夕暮れ）",
		startBar: 56,
		stage: { bgColor: "#7c6f84", bgDim: 0.2 },
...
export const STAGE_CAST_PRESET: MvPresetEntry = {
	...
	name: "ステージ整列＋コード進行",
	description:
		"風景の前にドット絵キャラが並び、8小節ごとに1体ずつ仲間が増えていく。下に歌詞の帯と64小節ぶんのコード進行バー。",
	swapHint:
		"背景に風景の1枚絵を入れ、キャラをあなたのドット絵に差し替えてください。コードは「レイヤー」タブのコード進行バーで編集できます。",
