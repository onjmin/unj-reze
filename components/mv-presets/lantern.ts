...
const LAYERS: MvLayer[] = [
	...
	// ══ 中央。「1小節ぶんの拡大ロール」という以前の理解ごと要見直し ══════════
	// コマ送り(0.25秒刻み)で見ると、ここは滑らかに動くビジュアライザーではなく、
	// ブラケット/中央四角/左右の小さい四角など、離散的な図形が数百ms単位で
	// 次々に差し替わって見える構造だった（詳細は zoom_contact 系の検証画像）。
	// pianoRoll(トラックを絞らない=複数ノートが同時に散らばる)のままだと、
	// 実際には無い「散らばったノート群」を描いてしまう。トラックを1つに絞って
	// 散らばりだけは抑えたが、これはあくまで応急処置——ブラケットが開閉したり
	// 図形そのものが差し替わる動きは、今のpianoRoll方式では原理的に作れない。
	// 本来はアイコンを差し替える専用の描画（現状のエンジンに無い）が要る。
	{
		kind: "visualizer",
		id: "zoom",
		style: "pianoRoll",
		...
		// 旋律だけに絞る。複数トラックを重ねると同時に複数ノートが別位置に散らばり、
		// 参考動画には無い「ばらけたピアノロール」になっていた。
		tracks: [0],
		amount: 1,
		thickness: 1,
	...
	// 「発音のたびに膨らんで戻る」というmodulatorを一度足したが、これも px計測で
	// 裏取りしていない当てずっぽうだった（lantern同様に撤回）。実際の枠の動きは
	// ブラケットの開閉のような形の差し替えで、サイズの脈動では近似しきれない——
	// 直すなら zoom 側と同じくアイコン差し替えの仕組みが要る。今は静止に戻す。
	{
		kind: "shape",
		id: "zoom-frame",
		form: "bar",
