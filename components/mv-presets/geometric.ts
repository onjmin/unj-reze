...
import { cloneManifest, mvTrack, rest, type MvPresetEntry } from './shared';
...
const MELODY = [
  ...
  ...rest(2), 'a2 r2', 'r1', ...rest(2), 'e2 r2', 'r1',
  // A
  'e a b a', 'a2 r2', 'g a b a', 'g2 r2', 'e a b a', 'a b >c< b', 'a1', 'r1',
  // A'
  'e a b a', 'a2 r2', 'b >c d c<', 'b2 r2', 'a b >c< b', 'g a b a', 'e1', 'r1',
  // B
  'a b >c< b', '>c d e d<', 'b >c d c<', 'a2 b2', 'g a b >c<', 'b a g e', 'a1', 'a1',
  // サビ
  '>e d c< b', 'a b >c< b', '>d c< b a', 'g a b >c<', '>e2 d2<', '>c< b a g', 'a1', 'a1',
  // 間奏
  'r1', 'a2 r2', 'r1', 'e2 r2', 'r1', 'g2 r2', ...rest(2),
  // A''
  'e a b a', 'a2 r2', 'b >c d c<', 'b2 r2', 'a b >c< b', 'g a b a', 'e1', 'r1',
  // アウトロ
  'e a b a', 'a2 r2', 'e2 g2', 'a1', 'r1', 'a1', ...rest(2),
];
...
const SECTIONS: MvSection[] = [
  { id: 'intro', label: 'イントロ（点）', startBar: 0, stage: { bgColor: '#0e423c' } },
  { id: 'a', label: '輪', startBar: 8, stage: { bgColor: '#0e423c' }, transition: { style: 'fade', beats: 0.75 } },
  { id: 'b', label: '二重丸', startBar: 16, stage: { bgColor: '#0d3b3f' }, transition: { style: 'fade', beats: 0.75 } },
  { id: 'c', label: '四角とひし形', startBar: 24, stage: { bgColor: '#123f36' }, transition: { style: 'fade', beats: 0.75 } },
  // サビだけ地の色が上がり、転換も白で抜ける
  { id: 'sabi', label: 'サビ（十字）', startBar: 32, stage: { bgColor: '#12574d' }, transition: { style: 'flash', beats: 0.75 } },
  { id: 'inter', label: '間奏（同心円）', startBar: 40, stage: { bgColor: '#0b3a36' }, transition: { style: 'fade', beats: 1 } },
  { id: 'd', label: '三角', startBar: 48, stage: { bgColor: '#103f45' }, transition: { style: 'fade', beats: 0.75 } },
  { id: 'end', label: 'アウトロ（点へ戻る）', startBar: 56, stage: { bgColor: '#081f1d' }, transition: { style: 'fade', beats: 1.5 } },
];
...
const LAYERS: MvLayer[] = [
  ...
  // ── 二重の輪＋ベースで灯る芯（的のかたち）────────────────────
  motif({ id: 'ring-b', size: 26, thickness: 1.4, count: 2, spread: 22, sections: ['b'] }),
  motif({
    id: 'core-b',
    form: 'circle',
    size: 12,
  ...
  motif({
    ...
    modulators: [
      ...
      { source: 'trackEnergy', track: 0, target: 'size', op: 'add', amount: 10 },
    ],
  }),
  motif({
    id: 'ring-s',
  ...
  motif({
    ...
    modulators: [
      ...
      { source: 'trackOnset', track: 1, target: 'thickness', op: 'add', amount: 1.8 },
    ],
  }),

  // ── 三角。1つは上向き、もう1つは伏せて重ねる ─────────────────
  motif({ id: 'tri-d', form: 'triangle', size: 40, thickness: 1.2, sections: ['d'] }),
  motif({
    id: 'tri-d2',
    form: 'triangle',
    size: 40,
...
export const GEOMETRIC_PRESET: MvPresetEntry = {
  kind: 'geometric',
  name: 'ジオメトリック',
  description: '暗い画面の中央にひとつだけ置かれた白い図形が、音の瞬間だけ濃くなる。8小節ごとに点→輪→二重丸→十字→同心円…と姿を変える64小節構成。',
  swapHint: '素材は要りません。「場面」タブで場面ごとの地の色と切り替え方を、「レイヤー」タブで各場面のモチーフを差し替えられます。',
