...
const MusicShareModal = dynamic(() => import("./MusicShareModal"), {
	ssr: false,
});
...
export default function ProfileView({
	...
	const slug = useMemo(
		() => cleanUserId.match(/[a-zA-Z0-9]+$/)?.[0] || cleanUserId,
		[cleanUserId],
	);
	...
	const [followListTab, setFollowListTab] = useState<FollowListTab | null>(
		null,
	);
	...
	const handleCropConfirm = async (dataUrl: string) => {
		...
		try {
			...
			// 更新対象はサーバー側がセッションから決めるので、誰の分かはここでは指定しない。
			// 表示名も送らない：avatarInfo.username は画面表示用のラベルで、
			// 保存すると本名として書き込まれ slug（所有者キー）まで変わってしまう。
			await api.auth.updateProfile({ avatarUrl: res.url });
	...
	const handleSaveBio = async () => {
		...
		setEditError(null);
		try {
			await api.auth.updateProfile({ bio: editBio.trim() });
			setBio(editBio.trim());
	...
	useEffect(() => {
		const applyVolume = () => {
			if (oshiAudioRef.current)
				oshiAudioRef.current.volume = applyMasterVolume(100) / 100;
		};
	...
	const threads = useMemo(
		() => myPosts.filter((p) => p.id === p.threadId),
		[myPosts],
	);
		...
		() =>
			myPosts.filter(
				(p) =>
					...
					p.hasMv ||
					p.hasMml ||
					!!extractMmlFromContent(p.content) ||
					!!extractChordsFromContent(p.content),
			),
		[myPosts],
	);

	const totalHearts = useMemo(
		() => myPosts.reduce((s, p) => s + Number(p.heartsTotal), 0),
		[myPosts],
	);
	const totalLikes = useMemo(
		() => myPosts.reduce((s, p) => s + Number(p.likes), 0),
		[myPosts],
	);
	const totalDislikes = useMemo(
		() => myPosts.reduce((s, p) => s + Number(p.dislikes), 0),
		[myPosts],
	);
		...
		<div className="flex flex-col min-h-full">
			<div className="p-4 border-b border-gray-800 bg-gradient-to-b from-gray-100/[0.03] to-transparent shrink-0">
				<div className="flex items-start space-x-3.5 mb-3">
					<div className="relative shrink-0">
						<div
							onClick={
								isSelf ? () => avatarFileInputRef.current?.click() : undefined
							}
						...
						>
							{avatarUrl ? (
								...
								(() => {
									...
									return (
										<AvatarIcon className="w-8 h-8 text-white/40 leading-none" />
									);
								})()
							)}
						</div>
					...
					<div className="mt-3.5">
						...
						{oshiItems.length > 0 ? (
							<div className="flex space-x-2.5 overflow-x-auto scrollbar-none pb-1">
								{oshiItems.map((item) => (
									<div key={item.id} className="shrink-0 w-24 select-none">
										<div className="relative w-24 h-24 rounded-lg overflow-hidden bg-gray-800 border border-gray-800 flex items-center justify-center">
											...
											{item.previewUrl && (
												<button
													...
													aria-label={
														playingOshiId === item.id ? "一時停止" : "試聴する"
													}
												>
													{playingOshiId === item.id ? (
														<Pause size={12} />
										...
										<div className="text-[10px] text-gray-300 font-bold truncate mt-1">
											{item.title}
										</div>
										{item.subtitle && (
				...
				{stats.map((s) => {
					...
					return (
						...
						>
							...
							{isActive && (
								<span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-[3px] bg-[#a3e635] rounded-full" />
							)}
						</button>
					);
				})}
					...
					filteredPosts
						...
						.map((p) => {
							...
							return (
								<div
									key={p.id}
									className="flex relative transition-all hover:bg-gray-100/5"
								>
											...
											<div className="flex justify-between items-baseline mb-0.5">
												<div className="flex items-baseline space-x-1.5">
													<span className="font-bold text-xs text-gray-200">
														{pAvatarInfo.username}
													</span>
													{(() => {
														const opt = ORIGIN_TYPE_OPTIONS.find(
															(o) => o.value === p.originType,
														);
														return opt ? (
															<span
			...
			{isMusicModalOpen && (
				<MusicShareModal
					...
					onRemove={(id) =>
						setOshiItems((prev) => prev.filter((o) => o.id !== id))
					}
