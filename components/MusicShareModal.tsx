...
const TABS: { id: OshiItemKind; entity: 'song' | 'album' | 'musicArtist'; label: string }[] = [
...
export default function MusicShareModal({ userSlug, oshiItems, onAdd, onRemove, onClose }: MusicShareModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center overflow-y-auto bg-black/70 backdrop-blur-sm p-3 pt-6" onClick={(e) => e.stopPropagation()}>
      <div className="relative w-full max-w-md bg-[#0b0e14] rounded-2xl border border-gray-800 shadow-2xl flex flex-col animate-fade-in-up max-h-[calc(100vh-3rem)]">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5 min-h-[240px]">
          {loading && (
            <div className="text-center text-[11px] text-gray-600 py-6">検索中...</div>
          )}
          {!loading && term.trim() && results.length === 0 && (
            <div className="text-center text-[11px] text-gray-600 py-6">見つかりませんでした</div>
          )}
          {!loading && results.map((r, i) => {
            ...
            const title = activeTab === 'artist' ? r.artistName : activeTab === 'album' ? r.collectionName : r.trackName;
            const subtitle = activeTab === 'artist' ? 'アーティスト' : activeTab === 'album' ? r.artistName : `${r.artistName || ''} · ${r.collectionName || ''}`;
            return (
              <div key={`${key}-${i}`} className="flex items-center gap-2.5 px-1.5 py-2 rounded-lg hover:bg-gray-100/5 transition-colors">
                <div className="relative w-11 h-11 rounded-lg overflow-hidden bg-gray-800 shrink-0">
                  {artwork && <img src={artwork} alt={title || ''} className="w-full h-full object-cover" />}
                  {activeTab !== 'artist' && r.previewUrl && (
                    ...
                    >
                      {playingUrl === r.previewUrl ? <Pause size={14} /> : <Play size={14} />}
                    </button>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-gray-200 font-bold truncate">{title}</div>
