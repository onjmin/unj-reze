...
export default function PostComposer({
	...
	const attachmentPreviews = (
		<>
			<div
				className={`grid transition-[grid-template-rows] duration-300 ease-out ${image ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
			>
			...
			{mml && (
				<div
					className={`relative mt-2 rounded-lg border border-pink-700/50 bg-pink-500/10 px-3 py-2 max-w-[280px] ${md ? "md:px-4 md:py-3 md:max-w-[420px]" : ""}`}
				>
					<div className="flex items-center justify-between mb-1">
						<span
							className={`font-bold text-pink-300 flex items-center gap-1 ${md ? "text-[10px] md:text-xs" : "text-[10px]"}`}
						>
							<Music size={12} />
							MMLを添付中（試聴できます）
						</span>
			...
			{gameDraft && (
				<div
					className={`relative mt-2 flex items-center gap-2 rounded-lg border border-yellow-700/50 bg-yellow-500/10 px-3 py-2 max-w-[280px] ${md ? "md:px-4 md:py-3 md:max-w-[420px]" : ""}`}
				>
			...
			{mvDraft && (
				<div
					className={`relative mt-2 flex items-center gap-2 rounded-lg border border-cyan-700/50 bg-cyan-500/10 px-3 py-2 max-w-[280px] ${md ? "md:px-4 md:py-3 md:max-w-[420px]" : ""}`}
				>
		...
		>
			{avatarUrl ? (
				...
				(() => {
					...
					return (
						<AvatarIcon
							className={`text-white/40 leading-none ${md ? "w-5 h-5 md:w-7 md:h-7" : "w-5 h-5"}`}
						/>
					);
				})()
			)}
	...
	const composerBody = (
		<>
			<div
				className={`flex items-start ${avatar ? (md ? "space-x-3 md:space-x-4" : "space-x-3") : ""}`}
			>
				{avatar}
				<div className="flex-1">
					...
					{uploadError && (
						<p
							className={`text-red-400 mt-1 ${md ? "text-[10px] md:text-xs" : "text-[10px]"}`}
						>
							{uploadError}
						</p>
					)}
