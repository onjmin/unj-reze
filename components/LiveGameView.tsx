...
export default function LiveGameView({ userId, sessionId }: Props) {
	...
	const { data: liveManifest } = useRemoteJson<GameManifestDraft>(
	...
	const fetchInfo = useCallback(
		(signal?: AbortSignal) => {
			return fetch(`/api/games/live?userId=${encodeURIComponent(userId)}`, {
				signal,
			})
	...
	useRealtimeSubscription(
		...
		useCallback((msg) => {
			...
			setDanmakuComments((prev) => [
				...prev,
				`${getAvatarInfo(r.displayName).username}: ${r.content}`,
			]);
		}, []),
				...
				try {
					...
					if (res.ok) {
						...
						if (!commentBaselineSetRef.current) {
							...
							commentLastIdRef.current = replies.reduce(
								(max, r) => Math.max(max, decodeId(r.id) || 0),
								0,
							);
							return;
						}
						const newOnes = replies.filter(
							(r) => (decodeId(r.id) || 0) > commentLastIdRef.current,
						);
						if (newOnes.length > 0) {
							commentLastIdRef.current = Math.max(
				...
				{liveManifest && info ? (
					<GameMaker
						...
						onComment={async (text, displayName) => {
							...
							setDanmakuComments((prev) => [
								...prev,
								`${getAvatarInfo(displayName).username}: ${text}`,
							]);
							try {
						...
						info.nextCandidates.map((c) => {
							...
							return (
								...
								>
									...
									<span
										className={`shrink-0 font-mono tabular-nums ${voted ? "text-yellow-400" : "text-gray-500"}`}
									>
										{voted ? "✓ " : ""}
										{c.votes}票
									</span>
