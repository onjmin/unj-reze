...
export default function FollowListSheet({
  ...
  const handleToggleFollow = async (target: FollowUser) => {
    ...
    setUsers(prev => prev.map(u => (u.slug === target.slug ? { ...u, isFollowing: !wasFollowing } : u)));
    try {
  ...
  const tabBtn = (id: FollowListTab, label: string, count: number) => {
    ...
    return (
      ...
      >
        ...
        {active && <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-[3px] bg-[#a3e635] rounded-full" />}
      </button>
    );
  };
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <div className="pt-2.5 pb-1 shrink-0 flex justify-center cursor-pointer" onClick={onClose}>
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain divide-y divide-gray-800/60">
          {loading ? (
            ...
            <div className="p-12 text-center text-xs text-gray-600 flex flex-col items-center gap-2">
              ...
              <span>{tab === 'followers' ? 'フォロワーはまだいません' : 'まだ誰もフォローしていません'}</span>
            </div>
          ) : (
            users.map(u => {
              ...
              return (
                <div
                  key={u.slug}
                  onClick={() => {
                    cacheProfileSeed({ slug: u.slug, displayName: u.displayName, avatarUrl: u.avatarUrl });
                    onClose();
                ...
                >
                  ...
                  >
                    {u.avatarUrl ? (
                      <img src={u.avatarUrl} alt={info.username} className="w-full h-full object-cover" />
                    ) : (
                      <info.Icon className="w-5 h-5 text-white/40" />
                    )}
                  </div>
                  <span className="flex-1 min-w-0 text-xs font-bold text-gray-200 truncate">{info.username}</span>
                  {viewerId && !u.isSelf && (
                    <button
