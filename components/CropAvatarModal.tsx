...
export default function CropAvatarModal({ imageSrc, onCancel, onConfirm }: CropAvatarModalProps) {
  ...
  const clampPan = useCallback((next: { x: number; y: number }, currentZoom: number) => {
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (pointersRef.current.size === 1) {
      ...
      dragStartRef.current = { x: only[1].x, y: only[1].y, panX: pan.x, panY: pan.y };
    }
  };
