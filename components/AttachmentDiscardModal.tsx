...
export default function AttachmentDiscardModal({ onClose, onConfirm, discardType }: AttachmentDiscardModalProps) {
  const assetName = DISCARD_TYPE_NAMES[discardType] || '添付ファイル';

  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center overflow-y-auto px-3 pt-24" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="relative w-full md:max-w-md bg-[#0b0e14] rounded-xl border border-gray-800 shadow-2xl p-5 flex flex-col space-y-4 animate-fade-in-up">
        ...
        <div className="text-sm text-gray-300 py-2 leading-relaxed">
          添付している<span className="font-bold text-yellow-400">{assetName}</span>が消えますがよろしいですか？
        </div>
