...
export default function MmlSource({ post, fallback = null, children }: Props) {
  ...
  const { data, loading, error } = useRemoteText(inline ? undefined : post.mmlUrl);

  const mml = inline || data;
  if (mml) return <>{children(mml)}</>;
  if (loading) return <>{fallback}</>;
  if (error) {
    return <div className="text-[11px] text-red-400">MMLの読み込みに失敗しました</div>;
  }
  return null;
}
 ...
 */
export function useMmlSource(post: Pick<Post, 'content' | 'mmlUrl'> | null | undefined) {
