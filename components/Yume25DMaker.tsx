...
export type Yume25DTool =
	...
	| "talk"
	| "erase";
export const YUME25D_TOOL_LABELS: Record<Yume25DTool, string> = {
	floor: "床",
	wall: "壁",
	sprite: "スプライト",
	start: "開始",
...
export const yume25dTexList = (
	l: Layout25D | undefined,
	kind: Tex25D["kind"],
): Tex25D[] =>
	l
		? Object.values(l.textures)
				.filter((t) => t.kind === kind)
				.sort((a, b) => a.id - b.id)
		: [];

/** グリッドを新サイズへ切り詰め/拡張（拡張部は 0 埋め）。 */
export const yume25dResizeFloor = (
	floor: number[][],
	cols: number,
	rows: number,
): number[][] =>
	Array.from({ length: rows }, (_, r) =>
		Array.from({ length: cols }, (_, c) => floor[r]?.[c] ?? 0),
	);

const Yume25DMaker = forwardRef<Yume25DMakerHandle, Yume25DMakerProps>(
	function Yume25DMaker(
		{
			layout,
			onLayoutChange,
			isPlaying,
		...
		const [hpState, setHpState] = useState<{ hp: number; max: number } | null>(
			null,
		);
		/** 酸素ゲージ（layout.oxygen 有効時）。エンジンの onOxygenChange 通知で同期する。 */
		const [oxyState, setOxyState] = useState<{
			sec: number;
			max: number;
		} | null>(null);
		/** 空腹ゲージ（layout.hunger 有効時）。エンジンの onHungerChange 通知で同期する。 */
		const [hungerState, setHungerState] = useState<{
			pts: number;
			max: number;
		} | null>(null);
		/** Minecraft風の方向キー2回押しダッシュ。active はダッシュを発動させたキー（離すと解除）。 */
		const dashTapRef = useRef<{
			key: string;
			t: number;
			active: string | null;
		}>({ key: "", t: 0, active: null });
		...
		const centerScroll = (
			...
			clampScroll(
				col - Math.floor(VIEW_COLS / 2),
				row - Math.floor(VIEW_ROWS / 2),
				cols,
				rows,
			);
		...
		if (!isInitializedRef.current) {
			scrollRef.current = centerScroll(
				layout.start.col,
				layout.start.row,
				layout.cols,
				layout.rows,
		...
		useEffect(() => {
			...
			scrollRef.current = clampScroll(
				scrollRef.current.col,
				scrollRef.current.row,
				layout.cols,
				layout.rows,
			);
		...
		useEffect(() => {
			const walk = playerAppearance.spriteRef
				? parseWalkRef(playerAppearance.spriteRef)
				: null;
			const isAnimatableWalk = walk && walk.stdId !== "smc_json" && !walk.crop;
			const sheetUrl = isAnimatableWalk
				? (playerAppearance.spriteUrl ??
		...
		useEffect(() => {
			if (!playerIconCanvasRef.current)
				playerIconCanvasRef.current = document.createElement("canvas");
			playerIconCanvasRef.current.width = 64;
			playerIconCanvasRef.current.height = 64;
			drawPlayerIconCanvas(playerIconCanvasRef.current, playerAppearance, () =>
				setPlayerIconVersion((v) => v + 1),
			);
			setPlayerIconVersion((v) => v + 1);
		}, [
			playerAppearance.emoji,
			playerAppearance.color,
			playerAppearance.spriteUrl,
			playerAppearance.spriteRef,
		]);
		...
		useEffect(() => {
			...
			const onWheel = (ev: WheelEvent) => {
				ev.preventDefault();
				eng.adjustPovDistance(ev.deltaY * 0.0035);
			};
		...
		useEffect(() => {
			...
			if (!eng) return;
			eng.onDeath = onDeathRef.current
				? () => {
						onDeathRef.current?.();
					}
				: null;
		...
		useEffect(() => {
			engineRef.current?.setPlayerAppearance(playerAppearance);
		}, [playerAppearance]);
			...
			const setKey = (key: string, on: boolean): boolean => {
				...
				switch (key) {
					...
					case "Shift":
						...
						return true;
					case " ":
						if (!on) inp.flyUp = false;
						return true; // 押下側は down ハンドラが処理する
				}
				return false;
			};
			const down = (e: KeyboardEvent) => {
				...
				if (playing && !e.repeat && MOVE_KEYS.includes(e.key)) {
					...
					if (
						...
						now - dashTapRef.current.t < DASH_DOUBLE_TAP_MS
					) {
						const inp = engineRef.current?.input;
				...
				if (
					setKey(e.key, true) &&
					(e.key.startsWith("Arrow") || "wasdWASD".includes(e.key))
				)
					e.preventDefault();
			};
		...
		useImperativeHandle(
			ref,
			() => ({
				setFlyUp: (active: boolean) => {
					const inp = engineRef.current?.input;
					if (inp) inp.flyUp = active;
				},
				setFlyDown: (active: boolean) => {
					const inp = engineRef.current?.input;
					if (inp) inp.flyDown = active;
				},
				resetToStart: () => {
		...
		const holdProps = (
			...
			onPointerDown: (e: React.PointerEvent) => {
				...
				const inp = engineRef.current?.input;
				if (inp) inp[prop] = true;
			},
			onPointerUp: () => {
				const inp = engineRef.current?.input;
				if (inp) inp[prop] = false;
			},
			onPointerCancel: () => {
				const inp = engineRef.current?.input;
				if (inp) inp[prop] = false;
		...
		const prefersGeometry = (t: Yume25DTool) =>
			t === "floor" || t === "start" || t === "talk" || t === "erase";
		const computeGhost = useCallback(
			(clientX: number, clientY: number): GhostSpec | null => {
				const eng = engineRef.current,
					cv = glCanvasRef.current;
				...
				if (tool === "floor") {
					...
					return selFloor > 0
						? { kind: "floor", col: c, row: r, tex: selFloor }
						: { kind: "cell", col: c, row: r, level: 0, color: "#0d0a14" };
				}
				if (tool === "wall") {
				...
				if (tool === "start")
					return { kind: "cell", col: c, row: r, level: 0, color: "#7fffd4" };
				if (tool === "talk")
					return { kind: "cell", col: c, row: r, level: lv, color: "#38bdf8" };
		...
		useEffect(() => {
			...
			if (!eng.hover) {
				eng.input.flyUp = false;
				eng.input.flyDown = false;
			}
		...
		const glPointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
			...
			if (dx !== 0 || dy !== 0)
				engineRef.current?.turnBy(
					-dx * DRAG_TURN_SENSITIVITY,
					-dy * DRAG_TURN_SENSITIVITY,
				);
		...
		const glPointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
			...
			if (d.moved <= TAP_MAX_MOVE) {
				...
				const res = engineRef.current?.pickTarget(
					ndcX,
					ndcY,
					prefersGeometry(tool),
				);
				if (res) {
					const c = Math.floor(res.x),
						r = Math.floor(res.z);
		...
		const applyDpad = (e: React.PointerEvent<HTMLDivElement>) => {
			...
			if (dist < PAD_DEADZONE) {
				inp.forward = inp.back = inp.strafeL = inp.strafeR = false;
				return;
			}
			const nx = dx / dist,
				ny = dy / dist;
		...
		const dpadMove = (e: React.PointerEvent<HTMLDivElement>) => {
			if (dpadPointerRef.current === e.pointerId) applyDpad(e);
		};
		const dpadEnd = (e: React.PointerEvent<HTMLDivElement>) => {
			if (dpadPointerRef.current !== e.pointerId) return;
			dpadPointerRef.current = null;
			const inp = engineRef.current?.input;
			if (inp) inp.forward = inp.back = inp.strafeL = inp.strafeR = false;
		};
		...
		const draw2DCanvas = useCallback(() => {
			if (is3d) return;
			const cv = edCanvasRef.current;
			if (!cv) return;
			const ctx = cv.getContext("2d")!;
			const L = layoutRef.current;
			const scr = scrollRef.current;
			const cur = cursorRef.current;
			const offX = scr.col * CELL,
				offY = scr.row * CELL;
			...
			for (let c = 0; c <= L.cols; c++) {
				...
				ctx.lineTo(c * CELL + 0.5, L.rows * CELL);
				ctx.stroke();
			}
			for (let r = 0; r <= L.rows; r++) {
				ctx.beginPath();
				ctx.moveTo(0, r * CELL + 0.5);
				ctx.lineTo(L.cols * CELL, r * CELL + 0.5);
				ctx.stroke();
			}
			...
			const sorted = [...L.walls]
				.sort(
					(a, b) =>
						Math.abs((a.level ?? 0) - level) - Math.abs((b.level ?? 0) - level),
				)
				.reverse();
			for (const w of sorted) {
				...
				ctx.beginPath();
				if (w.dir === 0) {
					ctx.moveTo(w.col * CELL, w.row * CELL);
					ctx.lineTo((w.col + 1) * CELL, w.row * CELL);
				} else {
					ctx.moveTo(w.col * CELL, w.row * CELL);
					ctx.lineTo(w.col * CELL, (w.row + 1) * CELL);
				}
				ctx.stroke();
			}
			ctx.globalAlpha = 1;
			...
			for (const b of L.billboards) {
				...
				const lv = b.level ?? 0;
				const cx = (b.col + 0.5) * CELL,
					cy = (b.row + 0.5) * CELL;
				ctx.globalAlpha = lv === level ? 1 : 0.3;
				if (t?.emoji) {
					ctx.font = "18px serif";
					ctx.fillText(t.emoji, cx, cy + 1);
				} else {
			...
			if (!playing) {
				const cx = cur.col * CELL + CELL / 2,
					cy = cur.row * CELL + CELL / 2;
			...
			(
				...
				if (tool === "floor") {
					onLayoutChange((l) => {
						...
						const floor = l.floor.map((row, ri) =>
							ri === r ? row.map((v, ci) => (ci === c ? nextVal : v)) : row,
						);
						return { ...l, floor };
					});
					return;
				...
				if (tool === "wall") {
					...
					onLayoutChange((l) => {
						...
						if (hit && hit.tex === w.tex)
							return { ...l, walls: l.walls.filter((v) => v !== hit) }; // 同じ壁 → 取り除く
						if (hit)
							return { ...l, walls: l.walls.map((v) => (v === hit ? w : v)) }; // 別テクスチャ → 貼り替え
				...
				if (tool === "sprite") {
					onLayoutChange((l) => {
						const hit = l.billboards.find(
							(b) => b.col === c && b.row === r && (b.level ?? 0) === lv,
						);
						if (hit && hit.tex === selSprite) {
							// 同じスプライトを再クリック → 削除して取り除く
							return {
								...l,
								billboards: l.billboards.filter((b) => b !== hit),
							};
						}
						if (hit)
							return {
								...
								billboards: l.billboards.map((b) =>
									b === hit ? { ...b, tex: selSprite, dir: editDir } : b,
								),
							};
						return {
							...l,
				...
				if (tool === "start") {
					onLayoutChange((l) => {
						if (l.start.col === c && l.start.row === r)
							return {
								...l,
								start: { ...l.start, dir: ((l.start.dir + 1) % 4) as Dir4 },
							}; // 同マス再クリックで向き回転
				...
				onLayoutChange((l) => {
					const bb = l.billboards.find(
						(b) => b.col === c && b.row === r && (b.level ?? 0) === lv,
					);
					if (bb)
						return { ...l, billboards: l.billboards.filter((b) => b !== bb) };
					if (!isDrag) {
						const dists: [number, Dir4][] = [
							[fy, 0],
							[1 - fx, 1],
					...
					if (lv === 0 && (l.floor[r]?.[c] ?? 0) !== 0) {
						const floor = l.floor.map((row, ri) =>
							ri === r ? row.map((v, ci) => (ci === c ? 0 : v)) : row,
						);
						return { ...l, floor };
					}
					return l;
			...
			const loop = () => {
				...
				if (moving) {
					...
					const nextCol = Math.max(
						0,
						Math.min(L.cols - 1, cur.col + dx * speed * dt),
					);
					const nextRow = Math.max(
						0,
						Math.min(L.rows - 1, cur.row + dy * speed * dt),
					);
		...
		return (
			<div className="absolute inset-0 flex flex-col bg-black select-none">
				...
				{playing && !demo && hpState && (
					<div
						className="absolute top-2 right-2 z-20 flex flex-col items-end gap-0.5 pointer-events-none select-none"
					...
					>
						...
						{layout.hunger && hungerState && (
							<div className="flex gap-0.5 text-[13px] leading-none">
								{Array.from(
									{ length: Math.ceil(hungerState.max / 2) },
									(_, i) => {
				...
				{is3d && showControlGuide && !dialogue && (
					<div className="absolute inset-0 flex items-start justify-start p-3 z-20 pointer-events-none transition-opacity duration-300">
						<div className="bg-black/95 border-2 border-white/40 p-4 max-w-xs text-white text-center shadow-2xl pointer-events-auto font-pixel">
							<h4 className="text-violet-400 font-bold text-[11px] mb-2.5 tracking-widest">
								操作方法
							</h4>
							<div className="space-y-2 text-[10px] text-gray-300 text-left">
								<div className="flex items-center justify-between gap-4">
									<span>移動/ストレイフ</span>
									<span className="bg-gray-800 border border-gray-600 px-1 py-0.5 text-white">
										[W][A][S][D]
									</span>
								</div>
								<div className="flex items-center justify-between gap-4">
