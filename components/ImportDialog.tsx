...
interface ImportDialogProps {
  open: boolean;
  onClose: () => void;
  onImport: (image: HTMLImageElement, opts: { opacity: number; simple: boolean }) => void;
  walkMode: boolean;
  walkPresets: WalkPreset[];
}

export default function ImportDialog({ open, onClose, onImport, walkMode, walkPresets }: ImportDialogProps) {
    ...
    <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/50">
      <div className="bg-[#1a1b26] rounded-xl border border-gray-700 shadow-2xl w-[360px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-3">
          ...
          <div>
            <span className="text-[10px] text-gray-500 font-medium">ローカルファイル</span>
          ...
          {walkMode && detected && (
            <div className="text-[10px] text-green-400 bg-green-900/20 px-2 py-1 rounded">
              検出: {detected.label} ({detected.w}×{detected.h} / {detected.frames}fr / {detected.ways.length}方向)
            </div>
          )}
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-[10px] text-gray-400 w-8 text-right">{opacity}%</span>
            </div>
            {walkMode && (
              <label className="flex items-center gap-2">
                ...
                />
                <span className="text-[10px] text-gray-400">1枚絵として読み込む（スプライト分割しない）</span>
              </label>
            )}
          </div>
