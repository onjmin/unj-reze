...
export default function MediaGrid({ items }: MediaGridProps) {
  ...
  const sorted = useMemo(() => {
    ...
    else next.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    return next;
  }, [items, sort]);
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center py-20 bg-gray-900/5">
        ...
        <p className="text-sm font-bold text-gray-200">メディア付き投稿はまだありません。</p>
      </div>
    );
  }
        ...
        {sorted.map(post => (
          ...
          >
            ...
            {post.imageSrc?.toLowerCase().includes('.gif') && (
              <span className="absolute top-1 left-1 bg-black/70 text-white text-[9px] font-bold px-1.5 py-0.5 rounded">GIF</span>
            )}
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-1.5 py-1 flex items-center gap-2 text-[10px] text-white font-bold opacity-0 group-hover:opacity-100 transition-opacity">
              <span className="flex items-center gap-0.5"><ThumbsUp size={10} />{post.likes}</span>
              <span className="flex items-center gap-0.5"><ThumbsDown size={10} />{post.dislikes}</span>
              <span className="flex items-center gap-0.5"><MessageCircle size={10} />{post.repliesCount}</span>
            </div>
          </button>
        ))}
      </div>
