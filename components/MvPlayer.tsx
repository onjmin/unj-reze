...
import { type MvPlaybackHandle, startMvPlayback } from "@/lib/mv-audio";
import {
	MV_H,
	MV_STEPS_PER_BAR,
	MV_STEPS_PER_BEAT,
...
const MvPlayer = forwardRef<MvPlayerHandle, MvPlayerProps>(function MvPlayer(
		...
		(startStep?: number) => {
			...
			startMvPlayback(mml, songRef.current.lyricTrackIds, {
				...
				.then((playback) => {
					...
					const stepsPerMs =
						(((songRef.current.bpm || 120) / 60) * MV_STEPS_PER_BEAT) / 1000;
					...
					const loop = () => {
						...
						const step = Math.max(
							lastStep,
							tick.step + Math.max(0, now - tick.atMs) * stepsPerMs,
						);
						lastStep = step;
	...
	useEffect(
		() =>
			subscribeMasterVolume(() =>
				playbackRef.current?.setVolume(applyMasterVolume(100)),
			),
		[],
	);
	...
	const toggle = () => {
		if (playing) {
			...
			const currentStep = seekBarRef.current
				? Number(seekBarRef.current.value)
				: 0;
	...
	const handleSeek = useCallback(
		(e: React.ChangeEvent<HTMLInputElement>) => {
			...
			const stepsPerSec =
				((songRef.current.bpm || 120) / 60) * MV_STEPS_PER_BEAT;
	...
	return (
		<div
			className={`flex flex-col w-full overflow-hidden rounded-lg bg-black ${className ?? ""}`}
		>
			...
			{controls && hasMml && (
				<div className="flex shrink-0 items-center gap-3 border-t border-gray-800/80 bg-gray-950/90 px-3 py-2">
					...
					<input
						...
						max={Math.max(
							1,
							(songRef.current.totalBars || 1) * MV_STEPS_PER_BAR,
						)}
						step="1"
						disabled={!ready}
						defaultValue="0"
					...
					<span
						ref={timeDisplayRef}
						className="w-9 shrink-0 text-right font-mono text-[11px] text-gray-200"
					>
						0:00
					</span>
				</div>
