...
function LocalTileGrid({ sheet, onPick }: { sheet: LocalTileSheet; onPick: (res: PickResult) => void }) {
  const [cells, setCells] = useState<Cell[] | null>(() => cellsCache.get(sheet.id) ?? null);
            ...
            for (let row = 0; row < sheet.rows; row++) {
              ...
              if (sheet.hideIdenticalBlockRows && colEnd - colStart > 1) {
                ...
                for (let col = colStart + 1; col < colEnd; col++) {
                  if (cellHash(col, row) !== firstHash) { allSame = false; break; }
                }
                if (allSame) continue;
              }
              for (let col = colStart; col < colEnd; col++) {
                found.push({ idx: row * sheet.cols + col, opaque: isOpaque(col, row) });
              }
            }
          }
        } else {
  ...
  const filteredCells = ranges.length > 0
    ? cells.filter((c) => {
        const row = (c.idx / sheet.cols) | 0;
        return ranges.some((rg) => active.has(rg.row) && row >= rg.start && row <= rg.end);
      })
    : cells;
  ...
  return (
    <div className="flex flex-col gap-2">
      <p className="text-[10px] text-gray-600 px-0.5">{sheet.name} ・ 全{totalTiles}マス（16px）</p>
      {ranges.length > 0 && (
      ...
      <div
        ...
        style={{ gridTemplateColumns: `repeat(${sheet.scanBlockWidth ?? 8}, minmax(0, 1fr))` }}
      >
        {visible.map((c) => {
