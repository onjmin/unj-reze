...
function NumSlider({
	...
	return (
		<label className="flex items-center gap-2 text-[11px] py-0.5">
			...
			/>
			<span className="text-gray-200 w-9 text-right font-mono shrink-0">
				{value}
			</span>
		</label>
	);
			...
			<div className="flex gap-1 flex-wrap">
				{SPELL_PALETTE.map((col, i) => (
					<button
						...
						style={{
							background: col,
							outline: i === value ? "2px solid #fff" : undefined,
							outlineOffset: "1px",
						}}
...
function BlockCard({
	...
	const summary = (() => {
		switch (block.kind) {
			case "wait":
				return `${block.frames}f`;
			case "nway":
				return `${block.ways}方向 spd${block.speed} ±${block.spread}°`;
			case "aimed":
				return `spd${block.speed} ブレ±${block.jitter}°`;
			case "spiral":
				return `${block.ways}腕 rot${block.rotSpeed}°/f`;
			case "repeat":
				return `×${block.times}  (${block.body.length}ブロック)`;
		}
	})();
	...
	return (
		<div style={{ marginLeft: depth * 10 }} className="mb-1.5">
			{/* カード */}
			<div
				style={{ background: m.dim, borderLeft: `3px solid ${m.clr}` }}
				className="rounded-r-lg overflow-hidden select-none"
			>
				{/* ヘッダ行（タップで展開） */}
				<div
					className="flex items-center gap-2 px-2 py-2 cursor-pointer active:brightness-125 touch-manipulation"
					onClick={() => setOpen((o) => !o)}
				>
					...
					>
						{onMoveUp && (
							<button
								...
								className="p-1 text-gray-500 active:text-white touch-manipulation"
							>
								<ChevronUp size={13} />
							</button>
						)}
						...
						<button
							...
							className="p-1 text-red-500 active:text-red-300 touch-manipulation"
						>
							<Trash2 size={13} />
						</button>
					</div>
					...
					<div className="px-3 pb-3 pt-1 space-y-1 border-t border-white/10">
						...
						{(block.kind === "nway" ||
							...
							<>
								<NumSlider
									...
									max={8}
									step={0.1}
									onChange={(v) => upd({ speed: +v.toFixed(1) })}
								/>
								<ColorRow
									value={block.color}
						...
						{block.kind === "spiral" && (
							<>
								...
								<NumSlider
									label="回転速度"
									value={block.rotSpeed}
									min={1}
									max={30}
									onChange={(v) => upd({ rotSpeed: v })}
								/>
							</>
						)}
						{block.kind === "repeat" && (
				...
				<div className="mt-1">
					{block.body.map((child, i) => (
						<BlockCard
							...
							depth={depth + 1}
							onUpdate={(nb) =>
								upd({ body: block.body.map((b, j) => (j === i ? nb : b)) })
							}
							onDelete={() =>
								upd({ body: block.body.filter((_, j) => j !== i) })
							}
							onMoveUp={
								i > 0
									? () => {
					...
					<div
						...
						className="flex gap-1 flex-wrap mt-1 mb-2"
					>
						{(["wait", "nway", "aimed", "spiral"] as SpellBlockKind[]).map(
							(k) => (
								<button
									key={k}
									onClick={() =>
										upd({ body: [...block.body, defaultBlock(k)] })
									}
...
export default function SpellEditor({ blocks, onChange }: Props) {
	const upd = (i: number, nb: SpellBlock) =>
		onChange(blocks.map((b, j) => (j === i ? nb : b)));
	const del = (i: number) => onChange(blocks.filter((_, j) => j !== i));
	const add = (kind: SpellBlockKind) =>
		onChange([...blocks, defaultBlock(kind)]);
