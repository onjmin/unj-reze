...
import { searchSpriteSheets, searchSprites, getSpriteById, spriteUrl, type SpriteItem, type SpriteSheetItem, type SpriteSheetMember } from '@/lib/rpgen-assets';
...
const cache: BrowserCache = {
  query: '', submitted: '', sheets: [], items: [], page: 1, pages: 1, total: 0, open: null, shown: SPRITES_PER_CHUNK,
};
...
export default function SpriteSheetBrowser({ onPick }: SpriteSheetBrowserProps) {
  ...
  useEffect(() => {
    ...
    cache.page = page; cache.pages = pages; cache.total = total;
    cache.open = open; cache.shown = shown;
  });

  // シートを開いている間、表示中メンバーのうち名前未取得のものだけ単体詳細を引いて補完する
  useEffect(() => {
    if (!open) return;
    const ids = open.sprite_ids.slice(0, shown).map((m) => m.id).filter((id) => !spriteNameCache.has(id));
    if (ids.length === 0) return;
    const ctrl = new AbortController();
    Promise.all(ids.map((id) =>
      getSpriteById(id, ctrl.signal).then((res) => { if (res?.name) spriteNameCache.set(id, res.name); }),
  ...
  useEffect(() => {
    ...
    setError(false);
    const req = submitted
      ? searchSprites({ q: submitted, page, limit: SHEETS_PER_PAGE, signal: ctrl.signal }).then((res) => {
          setItems((prev) => (page === 1 ? res.data : [...prev, ...res.data]));
          setPages(res.meta.pages);
          setTotal(res.meta.total);
        })
      : searchSpriteSheets({ q: submitted, page, limit: SHEETS_PER_PAGE, signal: ctrl.signal }).then((res) => {
  ...
  const pick = (m: SpriteSheetMember, sheetName: string) =>
    onPick({ ref: `url:${spriteUrl(m.id)}`, url: spriteUrl(m.id), label: spriteNameCache.get(m.id) || sheetName || `素材 (${m.id})` });

  const pickItem = (item: SpriteItem) =>
    onPick({ ref: `url:${spriteUrl(item.id)}`, url: spriteUrl(item.id), label: item.name || `素材 #${item.no}` });
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-[10px] text-gray-600 px-0.5">
        {searching ? <>素材を検索{total > 0 && <> ・{total}件</>}</> : <>人がまとめた素材集{total > 0 && <> ・{total}まとめ</>}</>}
