...
const parseMap = (rows: string[]): number[][] =>
	rows.map((r) => [...r].map((ch) => M[ch] ?? 0));
...
const scene1: SceneDef = {
	...
	objects: [
		...
		newObject({
			...
			name: "ライト博士",
			message:
				"ロック、Xでバスター、Zでジャンプじゃ。武器を拾ったらEで切り替えられるぞ。トゲに触れると一発でやられるから気をつけるんじゃ！",
			spriteRef: wr(SPR.light),
			spriteUrl: sa(SPR.light),
		}),
		// 歩兵ロボ（メットール型）
		newObject({
			emoji: "🪖",
			...
			bullet: "none",
			atk: 2,
			spriteRef: wr(SPR.metall),
			spriteUrl: sa(SPR.metall),
		}),
		newObject({
			emoji: "🪖",
			col: 13,
		...
		newObject({
			...
			bulletColor: "#ffaa00",
			name: "スナイパージョー",
			spriteRef: wr(SPR.joe),
			spriteUrl: sa(SPR.joe),
		}),
		...
		newObject({
			...
			bulletSpeed: 3.2,
			bulletColor: "#ff6644",
			spriteRef: wr(SPR.turret),
			spriteUrl: sa(SPR.turret),
		}),
		newObject({
			emoji: "👾",
			col: 26,
			...
			bulletColor: "#ff9900",
			bulletSpeed: 3,
			spriteRef: wr(SPR.hopper),
			spriteUrl: sa(SPR.hopper),
		}),
		// 機械虫（終盤の地上）
		newObject({
			emoji: "🐞",
			...
			atk: 2,
			name: "機械虫",
			spriteRef: wr(SPR.bug),
			spriteUrl: sa(SPR.bug),
		}),
		// アイテム：足場の上のエネルギーカプセル
...
const scene2: SceneDef = {
	...
	objects: [
		...
		newObject({
			...
			bulletColor: "#ffcc00",
			name: "スナイパーアーマー",
			spriteRef: wr(SPR.tank),
			spriteUrl: sa(SPR.tank),
		}),
		// 中段足場のクモ砲台
		newObject({
			emoji: "🕷️",
			...
			bulletSpeed: 2.5,
			bulletColor: "#ff9900",
			spriteRef: wr(SPR.spider),
			spriteUrl: sa(SPR.spider),
		}),
		newObject({
			emoji: "🕷️",
			col: 24,
...
const bossCut: SceneDef = {
	...
	exits: { right: "stage4" },
	bgm: {
		ref: "https://www.youtube.com/watch?v=uB1kNcqPe2U",
		src: "https://www.youtube.com/watch?v=uB1kNcqPe2U",
		type: "youtube",
	},
	objects: [
		newObject({
			...
			isBoss: true,
			name: "カットマン",
			spriteRef: wr(SPR.cutman),
			spriteUrl: sa(SPR.cutman),
			outroDialogue: [
				{
					speaker: "カットマン",
					emoji: "✂️",
					text: "ぐおおっ……そんな、バスターに敗れるとは……！",
				},
				{
					speaker: "ロック",
					emoji: "🤖",
					side: "right",
					text: "ローリングカッターはもらっていくよ！",
				},
			],
		}),
		// ボスの奥の報酬
...
const bossMetal: SceneDef = {
	...
	exits: { right: "wily1" },
	bgm: {
		ref: `direct:${megamanMusicUrl("metalman")}`,
		src: megamanMusicUrl("metalman"),
		type: "direct",
	},
	objects: [
		newObject({
			...
			isBoss: true,
			name: "メタルマン",
			spriteRef: wr(SPR.metalman),
			spriteUrl: sa(SPR.metalman),
			outroDialogue: [
				{
					speaker: "メタルマン",
					emoji: "⚙️",
					text: "バカな……このメタルブレードが……通じない……",
				},
				{
					speaker: "ロック",
					emoji: "🤖",
...
const wily1: SceneDef = {
	...
	objects: [
		...
		newObject({
			...
			atk: 4,
			name: "ガードロボ",
			spriteRef: wr(SPR.robot),
			spriteUrl: sa(SPR.robot),
		}),
		newObject({
			emoji: "🐞",
			col: 27,
...
const bossWily: SceneDef = {
	...
	objects: [
		...
		newObject({
			...
			name: "Dr.ワイリー",
			message:
				"よ、よくぞここまで来おったな！ワイリーマシン1号の力を見せてやるわい！",
			spriteRef: wr(SPR.wily),
			spriteUrl: sa(SPR.wily),
		}),
		newObject({
			emoji: "🛸",
			col: 11,
			...
			outroDialogue: [
				{
					speaker: "Dr.ワイリー",
					emoji: "🧑‍🔬",
					text: "ワイリーマシンが……！　ま、まいった！ゆるしてくれ〜！",
				},
				{
					speaker: "ロック",
					emoji: "🤖",
					side: "right",
					text: "もう悪さはさせない。扉の先へ進もう。",
				},
			],
		}),
	],
...
export const rockman: PresetData = {
	...
	player: {
		...
		spriteRef: wr(SPR.rock),
		spriteUrl: sa(SPR.rock),
	},
	tiles,
	...
	items: [
		{
			...
			name: "ロックバスター",
			emoji: "🔫",
			description: "初期装備。エネルギー無限の通常弾",
		},
		{
			id: "energyCapsule",
			name: "エネルギーカプセル",
			emoji: "💊",
			description: "ライフを少し回復する小さなカプセル",
		},
		{
			id: "smallEnergyTank",
			name: "小エネルギータンク",
			emoji: "🩹",
			description: "ライフを半分回復する中型タンク",
		},
		{
			id: "energyTank",
			name: "エネルギータンク",
			emoji: "🔋",
			description: "ライフを全回復する大型タンク",
		},
		{
			id: "weaponTank",
			name: "武器エネルギータンク",
			emoji: "🔵",
			description: "全武器のエネルギーを全回復する",
		},
		{
			id: "smallWeaponTank",
			name: "小武器エネルギータンク",
			emoji: "🔹",
			description: "選択中の武器エネルギーを半分回復する",
		},
		// 特殊武器（id はエンジン側の弾種と対応。E キーで切り替え）
		{
			...
			name: "ローリングカッター",
			emoji: "✂️",
			description: "カットマンの武器。3方向にカッターを放つ",
		},
		{
			id: "metalBlade",
			name: "メタルブレード",
			emoji: "🗡️",
			description: "メタルマンの武器。8方向に回転ブレードを投げる",
		},
		{
			id: "crashBomb",
			name: "クラッシュボム",
			emoji: "💣",
			description: "ワイリー製の大型爆弾を撃ち出す",
		},
	],
	titleScreen: {
		enabled: true,
	...
	ending: {
		...
		message:
			"ワイリーマシンを撃破し、世界に平和が戻った。\nロックの戦いは、まだ始まったばかり…",
		textColor: "#4488ff",
	},
	bgm: {
		ref: "https://www.youtube.com/watch?v=wgP_PK_umKM",
		src: "https://www.youtube.com/watch?v=wgP_PK_umKM",
		type: "youtube",
	},
	bossBgm: {
		ref: "https://www.youtube.com/watch?v=uB1kNcqPe2U",
	...
	sfx: {
		shot: {
			ref: `direct:${megamanSfxUrl("shot")}`,
			src: megamanSfxUrl("shot"),
			type: "direct",
		},
		jump: {
			ref: `direct:${megamanSfxUrl("jump")}`,
			src: megamanSfxUrl("jump"),
			type: "direct",
		},
		clear: {
			ref: `direct:${megamanSfxUrl("energyFill")}`,
			src: megamanSfxUrl("energyFill"),
			type: "direct",
		},
		damage: {
			ref: `direct:${megamanSfxUrl("damage")}`,
			src: megamanSfxUrl("damage"),
			type: "direct",
		},
	},
};
