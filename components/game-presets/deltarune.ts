...
const npc = (
	...
	row: number,
	message: string,
	sprite?: { spriteRef: string; spriteUrl: string },
) =>
	newObject({
		emoji,
		col,
...
const sceneField: SceneDef = {
	...
	bgm: {
		ref: `direct:${tldrMusicUrl("exForest")}`,
		src: tldrMusicUrl("exForest"),
		type: "direct",
	},
	objects: [
		// スージー（同行NPC・強気なコメント）
		npc(
			...
			20,
			3,
			"スージー「はっ、こんな くらい ところ ビビらせようったって そうはいかないっての」",
			symbolSprite(TLDR_PARTY_SPRITES.susie.idle),
		),
		// 宝物庫
...
const sceneTown: SceneDef = {
	...
	bgm: {
		ref: `direct:${tldrMusicUrl("exCity")}`,
		src: tldrMusicUrl("exCity"),
		type: "direct",
	},
	objects: [
		// 宿・回復ポイント
			...
			pages: [
				{
					...
					commands: [
						{
							type: "message",
							text: "宿の主人「やあ、旅の人。ゆっくり やすんで いきなよ」",
						},
						{ type: "restoreHp" },
						{ type: "restoreMp" },
			...
			pages: [
				{
					...
					commands: [
						{
							type: "message",
							text: "店員「いらっしゃい！ くらやみ製の 武具も おいてるよ」",
						},
					],
				},
			],
		...
		npc(
			...
			6,
			8,
			"ノエル「あ、あの……だいじょうぶ、ですか？ わたしで よければ おてつだい します……」",
			walkSprite(SPR.noelle),
		),
		// 看板イベント
		newObject({
			emoji: "🪧",
...
const sceneCastle: SceneDef = {
	...
	map: castleMap,
	bgm: {
		ref: `direct:${tldrMusicUrl("story")}`,
		src: tldrMusicUrl("story"),
		type: "direct",
	},
	objects: [
		// ラルセイの応援
		npc(
			...
			15,
			4,
			"ラルセイ「おうさまは きっと わかりあえる。\nでも……ゆだんは できないよ」",
			symbolSprite(TLDR_PARTY_SPRITES.ralsei.idle),
		),
		// 玉座の前・王とのやりとり
		newObject({
			emoji: "👑",
			...
			pages: [
				...
				},
				{
					...
					commands: [
						{
							type: "message",
							text: "おうさま「ようこそ、まよい人よ。\nこの せかいの ためにも……ここは とおせぬ」",
						},
						{
							type: "choice",
							text: "おうさま「たたかいたくは ないが、どうする？」",
							choices: [
								{
									label: "たたかう",
									commands: [
...
`.trim(),
			...
			outroDialogue: [
				...
				},
				{
					...
					emoji: "🐐",
					side: "right",
					text: "これで……くらやみの せかいに ひかりが もどるはず。",
				},
				{
					speaker: "スージー",
					emoji: "😈",
...
export const deltarune: PresetData = {
		...
		party: [
			{
				...
				emoji: "🙂",
				maxHp: 26,
				color: TLDR_PARTY_UI.kris.color,
				battleSprites: {
					...TLDR_PARTY_SPRITES.kris,
					icon: TLDR_PARTY_UI.kris.icon,
					iconHurt: TLDR_PARTY_UI.kris.iconHurt,
				},
			},
			...
			// （原作の攻撃×11＋魔力×5をこのゲームのダメージスケールに換算）。SEも同エンジンの専用音源。
			{
				...
				emoji: "😈",
				maxHp: 34,
				color: TLDR_PARTY_UI.susie.color,
				battleSprites: {
					...TLDR_PARTY_SPRITES.susie,
					icon: TLDR_PARTY_UI.susie.icon,
					iconHurt: TLDR_PARTY_UI.susie.iconHurt,
				},
				spells: [
					{
						name: "ルードバスター",
						tpCost: 50,
						power: 42,
						castSfxUrl: tldrSfxUrl("rudeBusterSwing"),
						hitSfxUrl: tldrSfxUrl("rudeBusterHit"),
					},
				],
			},
			{
				...
				emoji: "🐐",
				maxHp: 22,
				color: TLDR_PARTY_UI.ralsei.color,
				battleSprites: {
					...TLDR_PARTY_SPRITES.ralsei,
					icon: TLDR_PARTY_UI.ralsei.icon,
					iconHurt: TLDR_PARTY_UI.ralsei.iconHurt,
				},
				spells: [
	...
	items: [
		{
			...
			name: "ダークキャンディ",
			emoji: "🍬",
			description: "HPを 16 回復する。くらやみの あじ",
			category: "consumable",
			healHp: 16,
		},
		{
			...
			name: "クロワッサン",
			emoji: "🥐",
			description: "HPを 20 回復する。さくさく してる",
			category: "consumable",
			healHp: 20,
		},
		{
			...
			name: "チーズ",
			emoji: "🧀",
			description: "HPを 30 回復する。ねずみが すき",
			category: "consumable",
			healHp: 30,
		},
		{
		...
		},
		{
			...
			name: "くらやみの かんむり",
			emoji: "👑",
			description: "ふしぎな ちからを かんじる。しゅび力＋20",
			category: "armor",
			defBonus: 20,
		},
	],
	titleScreen: {
		...
		textColor: "#ffffff",
		menu: [{ kind: "newGame", label: "ぼうけんを はじめる" }],
	},
	ending: {
		enabled: true,
		heading: "つづく……",
		message:
			"くらやみの せかいに ひかりが もどった。\nまた いつか、この せかいで あおう。\n\n——きみの ぼうけんは、まだ はじまったばかり。",
		textColor: "#c9a53a",
	},
	bgm: {
		ref: `direct:${tldrMusicUrl("exSpawn")}`,
		src: tldrMusicUrl("exSpawn"),
		type: "direct",
	},
	battleBgm: {
		ref: `direct:${tldrMusicUrl("battle")}`,
		src: tldrMusicUrl("battle"),
		type: "direct",
	},
	bossBgm: {
		ref: `direct:${tldrMusicUrl("darkness")}`,
		src: tldrMusicUrl("darkness"),
		type: "direct",
	},
	sfx: {
		levelup: {
			ref: `direct:${tldrSfxUrl("levelup")}`,
			src: tldrSfxUrl("levelup"),
			type: "direct",
		},
		purchase: {
			ref: `direct:${tldrSfxUrl("uiSelect")}`,
			src: tldrSfxUrl("uiSelect"),
			type: "direct",
		},
		inn: {
			ref: `direct:${tldrSfxUrl("heal")}`,
			src: tldrSfxUrl("heal"),
			type: "direct",
		},
		save: {
			ref: `direct:${tldrSfxUrl("save")}`,
			src: tldrSfxUrl("save"),
			type: "direct",
		},
		damage: {
			ref: `direct:${tldrSfxUrl("hurt")}`,
			src: tldrSfxUrl("hurt"),
			type: "direct",
		},
		clear: {
			ref: `direct:${tldrSfxUrl("won")}`,
			src: tldrSfxUrl("won"),
			type: "direct",
		},
	},
