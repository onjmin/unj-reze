...
export interface ItemDef {
	...
	healHp?: number;
	healMp?: number;
	category?: "consumable" | "weapon" | "armor" | "key";
	consumable?: boolean;
	discardable?: boolean;
	useMessage?: string;
...
export type EventCommand =
	...
	| {
			...
			target: "materials" | "human" | "sprite" | "animation" | "wallpaper";
			id?: number;
			h?: number;
			s?: number;
			l?: number;
...
export const mkSpell = (
	kind: SpellBlockKind,
	over: Partial<SpellBlock> = {},
): SpellBlock => ({
	id: uid(),
	kind,
	frames: 30,
...
export interface TitleScreenConfig {
	...
	bgRef?: string;
	bgUrl?: string; // 背景画像（asset-ref / 解決済みURL）
...
export function expToNextLevel(
	level: number,
	growthType: GrowthType = "standard",
): number {
...
export interface PartyBattleSprites {
	...
	attackReady?: BattleSpriteAnim;
	attack?: BattleSpriteAnim;
	hurt?: BattleSpriteAnim;
	defend?: BattleSpriteAnim;
	defeat?: BattleSpriteAnim;
	act?: BattleSpriteAnim;
	spell?: BattleSpriteAnim;
	item?: BattleSpriteAnim;
	/** ステータスボックス用の顔アイコン（通常／被ダメージ時）。省略時は emoji で代用。 */
	icon?: BattleSpriteAnim;
	iconHurt?: BattleSpriteAnim;
}

/** 敵のバトル画面スプライト。設定すると戦闘オーバーレイで絵文字の代わりに描画される。
 *  hurt=被ダメージ演出中、spare=みのがし可能（敵意が消えた）とき。 */
export interface EnemyBattleSprite {
	idle: BattleSpriteAnim;
	hurt?: BattleSpriteAnim;
	spare?: BattleSpriteAnim;
}
...
export interface PartyMember {
	...
	def?: number;
	spells?: PartySpell[];
	battleSprites?: PartyBattleSprites /** メンバーカラー（HPバー・ボックス枠。省略時は白） */;
	color?: string;
}
...
export const normalizeWall25D = (
	...
	const w: Wall25D =
		dir === 2
			? { col, row: row + 1, dir: 0, tex } // 南辺 → 下セルの北辺
			: dir === 1
				? { col: col + 1, row, dir: 3, tex } // 東辺 → 右セルの西辺
				: { col, row, dir, tex };
...
const chestChipCrop = (col: number, row: number) =>
	`${col * 16},${row * 16},16,16`;
...
export const localSysTileUrl = (col: number, row: number) =>
	`${CHEST_CHIP_URL}#${col * 16},${row * 16},16,16`;
...
export const SYSTEM_TILE_TEMPLATES: SystemTileTemplate[] = [
	{
		...
		label: "シーン切替床",
		special: "warp",
		imageUrl: localSysTileUrl(15, 10),
		imageRef: `url:${localSysTileUrl(15, 10)}`,
		passable: true,
		color: "#7fd4ff",
	},
	{
		...
		label: "どく沼",
		special: "damage",
		imageUrl: localSysTileUrl(6, 0),
		imageRef: `url:${localSysTileUrl(6, 0)}`,
		passable: true,
		color: "#3f6d34",
	},
	{
		...
		label: "ダメージ床",
		special: "damage",
		imageUrl: localSysTileUrl(13, 7),
		imageRef: `url:${localSysTileUrl(13, 7)}`,
		passable: true,
		color: "#8a4a2a",
	},
	{
		...
		label: "つるつる床（↑）",
		special: "ice-up",
		imageUrl: localSysTileUrl(16, 13),
		imageRef: `url:${localSysTileUrl(16, 13)}`,
		passable: true,
		color: "#9fd8ea",
	},
	{
		...
		label: "つるつる床（→）",
		special: "ice-right",
		imageUrl: localSysTileUrl(17, 13),
		imageRef: `url:${localSysTileUrl(17, 13)}`,
		passable: true,
		color: "#9fd8ea",
	},
	{
		...
		label: "つるつる床（←）",
		special: "ice-left",
		imageUrl: localSysTileUrl(16, 14),
		imageRef: `url:${localSysTileUrl(16, 14)}`,
		passable: true,
		color: "#9fd8ea",
	},
	{
		...
		label: "つるつる床（↓）",
		special: "ice-down",
		imageUrl: localSysTileUrl(17, 14),
		imageRef: `url:${localSysTileUrl(17, 14)}`,
		passable: true,
		color: "#9fd8ea",
	},
];
...
export const chest = (
	...
	newObject({
		...
		behavior: "still",
		hazard: false,
		spriteRef: CHEST_SPRITE_CLOSED,
		spriteUrl: CHEST_CHIP_URL,
		altSpriteRef: CHEST_SPRITE_OPEN,
		altSpriteUrl: CHEST_CHIP_URL,
		pages: [
			{
				conditions: {},
				commands: [
...
export const convertMapToLayout25D = (
	src: PresetData,
	base: Layout25D,
): Layout25D => {
	...
	for (const tid of tileIds) {
		...
		const special =
			t.special === "damage" || t.special?.startsWith("ice-")
				? t.special
				: undefined;
		textures[floorTexId(tid)] = {
			...
			imageUrl: t.imageUrl,
			...(special ? { special } : {}),
			...(special === "damage" && t.damageAmount !== undefined
	...
	const isPassable = (c: number, r: number) =>
		effTile(c, r).def?.passable !== false;
	...
	for (let r = 0; r < rows; r++)
		for (let c = 0; c < cols; c++) {
			...
			for (const [dx, dy, dir] of DIRS) {
				...
				if (isPassable(nc, nr))
					walls.push(normalizeWall25D(c, r, dir, wallTexId(id)));
			}
		}
	...
	for (const o of src.objects) {
		...
		textures[nextId] = {
			id: nextId,
			name: o.name || o.emoji || "オブジェ",
			kind: "sprite",
			color: "#c8c8dc",
			emoji: o.emoji || undefined,
			imageRef: o.spriteRef,
			imageUrl: o.spriteUrl,
		};
		billboards.push({
			id: `cv-obj-${o.id}`,
			col: o.col,
	...
	const startCol = Math.max(
		0,
		Math.min(cols - 1, Math.round(src.player.start.x / TILE_SIZE)),
	);
	const startRow = Math.max(
		0,
		Math.min(rows - 1, Math.round(src.player.start.y / TILE_SIZE)),
	);
...
export const convertLayout25DToMap = (
	l: Layout25D,
): Pick<
	PresetData,
	"tiles" | "map" | "overlayMap" | "overheadMap" | "objects" | "scroll"
> & { startPx: { x: number; y: number } } => {
	...
	const map = Array.from({ length: l.rows }, (_, r) =>
		Array.from({ length: l.cols }, (_, c) => l.floor[r]?.[c] ?? 0),
	);
	const objects: ObjectDef[] = l.billboards
