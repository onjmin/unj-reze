...
export default function SMCAssetPanel({ onPick }: SMCAssetPanelProps) {
	...
	const [playerChar, setPlayerChar] = useState<"all" | "mario" | "luigi">(
		"mario",
	);
		...
		return (
			<div className="flex flex-col gap-3 min-h-[300px]">
					...
					<div className="flex flex-col gap-2 p-2 bg-gray-950 border border-gray-800 rounded-lg">
						<div className="flex items-center justify-between text-[10px] text-gray-500">
							...
							<div className="flex gap-1">
								<button
									className={activeFilterBtn(playerChar === "all")}
									onClick={() => setPlayerChar("all")}
								>
									すべて
								</button>
								<button
									className={activeFilterBtn(playerChar === "mario")}
									onClick={() => setPlayerChar("mario")}
								>
									マリオ
								</button>
								<button
									className={activeFilterBtn(playerChar === "luigi")}
									onClick={() => setPlayerChar("luigi")}
								>
									ルイージ
								</button>
							</div>
				...
				<div className="text-[10px] text-gray-500">
					該当アニメーション (
					{
						filteredAnims.filter((animName) => !failedAnims.has(animName))
							.length
					}
					件):
				</div>
				<div className="grid grid-cols-6 gap-2 overflow-y-auto max-h-[220px] scrollbar-none pr-1">
					{filteredAnims
						.filter((animName) => !failedAnims.has(animName))
						.map((animName) => (
							<button
								key={animName}
							...
							>
								<SMCSpritePreview
									...
									size={36}
									onError={() =>
										setFailedAnims((prev) => new Set(prev).add(animName))
									}
								/>
		...
		<div className="flex flex-col gap-3 min-h-[300px]">
			...
			{q && (
				<div className="flex flex-col gap-2">
					<div className="text-[10px] text-gray-500">
						検索結果 ({filteredSpriteNames.length}件):
					</div>
...
export function SMCSpritePreview({
	...
	useEffect(() => {
		...
		const urls = Array.from(
			new Set<string>(anim.frames.map((f) => resolveSMCUrl(f.image))),
		);
		Promise.all(
			urls.map((u) => loadImage(u).then((img) => imgByUrl.set(u, img))),
		)
			.then(() => {
				if (cancelled) return;
