...
export default function TopTabs({
	...
	return (
		<div className="sticky top-0 flex flex-col border-b border-gray-800 shrink-0 bg-[#0b0e14]/90 backdrop-blur-md z-20">
			...
			{activeTab === "everyone" && (
				<div className="flex px-2 py-1.5 text-xs bg-gray-100/5 text-gray-400 border-t border-gray-800/40">
					...
					>
						...
						{latestThreadCount > 0 && (
							<span
								className={`text-white text-[9px] rounded-full px-1.5 ml-1 font-bold ${feedSubMode === "threads" ? "bg-blue-600" : "bg-blue-600/50"}`}
							>
								{formatCount(latestThreadCount)}
							</span>
						)}
					</button>
					<button
					...
					>
						...
						{latestReplyCount > 0 && (
							<span
								className={`text-white text-[9px] rounded-full px-1.5 ml-1 font-bold ${feedSubMode === "replies" ? "bg-blue-600" : "bg-blue-600/50"}`}
							>
								{formatCount(latestReplyCount)}
							</span>
						)}
					</button>
					<button
					...
					>
						...
						{mediaCount > 0 && (
							<span
								className={`text-white text-[9px] rounded-full px-1.5 ml-1 font-bold ${feedSubMode === "media" ? "bg-blue-600" : "bg-blue-600/50"}`}
							>
