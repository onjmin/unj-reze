 ...
 */
export default function ShareButton({ url, text, size = 14, label, className }: ShareButtonProps) {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      >
        ...
        {(label || copied) && (
          <span className="text-[11px]">{copied ? 'コピーしました' : label}</span>
        )}
      </button>
      ...
      {open && (
        ...
        >
          {canNativeShare && (
            <button role="menuitem" onClick={handleNativeShare} className="flex items-center gap-2.5 w-full px-3 py-2 text-gray-300 hover:bg-gray-100/10 text-left transition-colors">
              <Share2 size={12} className="shrink-0" />
              <span>共有する…</span>
            </button>
          )}
          <button role="menuitem" onClick={handleX} className="flex items-center gap-2.5 w-full px-3 py-2 text-gray-300 hover:bg-gray-100/10 text-left transition-colors">
            <span className="shrink-0 w-3 text-center font-bold leading-none">𝕏</span>
            <span>Xでポスト</span>
          </button>
          <button role="menuitem" onClick={handleCopy} className="flex items-center gap-2.5 w-full px-3 py-2 text-gray-300 hover:bg-gray-100/10 text-left transition-colors">
