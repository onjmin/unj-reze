...
export const EMPTY_TLS_SIGNALS: TlsSignals = { ja4: null, ja3: null, version: null, cipher: null };
...
export function readTlsSignalsFromCf(cf: CfTlsProperties | undefined | null): TlsSignals {
...
export function tlsSignalsToHeaderEntries(signals: TlsSignals): [string, string | null][] {
  return [
...
export function assessTls(signals: TlsSignals): TlsAssessment {
  if (signals.ja4) {
    const known = KNOWN_BROWSER_JA4_PREFIXES.some(p => signals.ja4!.startsWith(p));
    return {
  ...
  if (signals.version) {
    if (!MODERN_TLS_VERSIONS.has(signals.version)) {
      return { verdict: 'non-browser', confidence: 'low', reason: `legacy-tls:${signals.version}` };
    }
