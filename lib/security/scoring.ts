import { kvGet, kvHDel, kvHGetAll, kvHSet, kvSetEx } from "@/lib/kv";
...
export async function scoreRequest(input: ScoreInput): Promise<ScoreResult> {
	...
	if (!turnstileOk) {
		...
		reasons.push(
			turnstileUnreachable ? "turnstile-unreachable" : "turnstile-failed",
		);
	}
	...
	const recentIpCount = Object.entries(ipEntries).filter(
		([, ts]) => now - Number(ts) < IP_HOP_WINDOW_MS,
	).length;
	...
	if (sessionId) {
		...
		await pruneStale(
			sessionMapKey,
			sessionEntries,
			SESSION_RECYCLE_WINDOW_MS,
			now,
		);
		const recentSessionCount = Object.entries(sessionEntries).filter(
			([, ts]) => now - Number(ts) < SESSION_RECYCLE_WINDOW_MS,
		).length;
