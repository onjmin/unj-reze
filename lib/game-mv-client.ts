...
function bgRefOf(manifest: GameManifestDraft): string | undefined {
	const ref = (manifest as { titleScreen?: { bgRef?: string } })?.titleScreen
		?.bgRef;
...
async function cleanupPrevious(
	prev: { deleteId?: string; deleteHash?: string } | undefined,
) {
...
export async function loadGame(
	...
	return {
		record,
		manifest: await fetchJson<GameManifestDraft>(record.manifestUrl),
	};
}
