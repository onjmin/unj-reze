...
export interface MvModulator {
	...
	phaseOffset?: number;
	/**
	 * source==='beat' 専用。`periodBeats`(発火の間隔＝周期)に対して、実際に
	 * 減衰にかける長さ（拍）を別に指定する。未指定なら従来どおり `periodBeats`
	 * 全体を使って減衰する（＝周期を伸ばすほど動きそのものがゆっくりになる）。
	 *
	 * これを指定すると「周期は長いが、鳴るときは常に短く素早く鳴る」——例えば
	 * 8拍おきに発火する要素でも、`attackBeats:1` なら毎回1拍ぶんの速さで
	 * サッと減衰し、残り7拍は静止する（8拍かけてゆっくり減衰するのではない）。
	 * シンバルのように長く尾を引かせたい少数派の演出だけ、`attackBeats` を
	 * 2〜4拍など長めに取る使い方もできる。
	 */
	attackBeats?: number;
	/**
	...
	shape?: "decay" | "bounce";
	/**
	 * source==='phrase' のとき、周期 `bars` を過ぎても折り返さず、値を終端で
	 * 固定する（既定false＝従来どおり `bars` ごとに1→0を繰り返すループ）。
	 *
	 * 特殊アレンジのポップイン／成長演出は「barRangeで見えている間ずっと
	 * ループする」つもりで `bars` を書いていない——`bars` は「育ちきるまでの
	 * 短い時間」のつもりで、layerのbarRange自体はそれよりずっと長い（次の幕まで
	 * 待つ）。その組み合わせで `bars` を過ぎるとエンベロープが折り返して
	 * 1→0→1→0…と再びズームし直して見えるバグになっていた
	 * （`bars` を layer の barRange 幅ぴったりに揃えれば従来どおりループ無しで
	 * 済むが、値を短く保ったまま「終わったら据え置く」を両立するにはこのフラグが
	 * 要る）。
	 */
	once?: boolean;
	target: MvModTarget;
