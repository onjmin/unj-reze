 ...
 */
export function isValidPayloadUrl(url: unknown, kind: 'mv' | 'game' | 'mml'): url is string {
  if (typeof url !== 'string' || url === '') return false;
  if (!UPLOADER_PUBLIC_URL) {
    // 未設定なら検証できない。通してしまうと素通しになるので落とす
    console.error('NEXT_PUBLIC_UPLOADER_PUBLIC_URL が未設定のため manifest URL を検証できません');
    return false;
  }
  let parsed: URL;
  try {
  ...
  return new RegExp(`^/${kind}/[0-9a-f]{16}\\.(json|mml|txt)$`).test(parsed.pathname);
}
 ...
 */
export function parseManifestRef(body: any, kind: 'mv' | 'game'): ParsedManifestRef | null {
...
export function parseMmlRef(
  ...
  if (body?.mmlUrl === undefined || body?.mmlUrl === null || body?.mmlUrl === '') return {};
