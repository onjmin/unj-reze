...
function toDtmLoopConfig(
	loop: BgmAsset["loop"],
): boolean | DtmLoopConfig | undefined {
...
interface SoundfontInstrument {
	play(
		note: string,
		when: number,
		opts: { gain: number; duration: number },
	): void;
}
...
class BgmManager {
	...
	async play(manifest: AssetManifest) {
		...
		if (
			this.currentKey &&
			this.currentKey.type === type &&
			this.currentKey.src === src &&
			this.current
		) {
	...
	stop() {
		...
		document
			.querySelectorAll(
				".bgm-youtube-container, .bgm-nicovideo-container, .bgm-soundcloud-container",
			)
	...
	private playDirect(
		...
		this.current = {
			...
			setBaseVolume: (v) => {
				audio.volume = (applyMasterVolume(v) / 100) * 0.6;
			},
			seek: (s) => {
				try {
					audio.currentTime = s;
	...
	private async playMidi(url: string, volume: number = 50) {
		...
		const player = new MP.Player((evt: MidiPlayerEvent) => {
			if (evt.name === "Note on" && evt.velocity > 0) {
				inst.play(evt.noteName, ctx.currentTime, {
					gain: (evt.velocity / 127) * (applyMasterVolume(volume) / 50),
					duration: 0.25,
				});
			}
		});
	...
	private playYoutube(
			...
			player = new window.YT!.Player("bgm-youtube-player", {
				...
				events: {
					onReady: (event) => {
						...
						if (finalStart > 0) {
							try {
								event.target.seekTo(finalStart, true);
							} catch {}
						}
		...
		this.current = {
			...
			setBaseVolume: (v) => {
				try {
					player?.setVolume(applyMasterVolume(v));
				} catch {}
			},
			seek: (s) => {
				try {
	...
	private playNicovideo(url: string, volume: number = 50) {
			...
			switch (e.data?.eventName) {
				...
				case "loadComplete": {
					postMsg({
						...
						data: { volume: (applyMasterVolume(volume) / 100) * 0.96 },
					});
					postMsg({ eventName: "play" });
					break;
				}
		...
		this.current = {
			...
			setBaseVolume: (v) => {
				postMsg({
					eventName: "volumeChange",
					data: { volume: (applyMasterVolume(v) / 100) * 0.96 },
				});
			},
		};
	}
	...
	private playSoundCloud(url: string, volume: number = 50) {
		...
		this.current = {
			...
			setBaseVolume: (v) => {
				try {
					widget?.setVolume(applyMasterVolume(v));
				} catch {}
			},
		};
	}
