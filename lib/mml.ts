...
export function tokenizeMmlTrack(body: string): MmlToken[] {
	...
	const raw =
		body.match(
			/l\d+\.?|o\d+|[<>]|v\d+|\[[^\]]*\]\d*\.?\d*|r\d*\.?\d*|[a-g][+#]?\d*\.?\d*/gi,
		) || [];
...
export function effectiveMmlVolume(mml: string, siteVolume: number): number {
	return Math.max(
		0,
		Math.min(100, Math.round((readMmlVolume(mml) * siteVolume) / 100)),
	);
}
...
export function playMml(
	...
	const safeTempo =
		typeof tempo === "number" && isFinite(tempo) && tempo > 0 ? tempo : 135;
	...
	tracks.forEach((t) => {
		const trackVol =
			typeof t.volume === "number" && isFinite(t.volume) ? t.volume : 100;
		t.notes.forEach((n) => {
	...
	allNotes.forEach((n) => {
		...
		const safeFreq =
			typeof n.freq === "number" && isFinite(n.freq) ? n.freq : 440;
 ...
 * MML行やURLなどを除去する。
 */
export function cleanContentForTrends(
