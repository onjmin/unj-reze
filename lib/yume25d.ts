...
const SHADER_GRADES: Record<
	"bsl" | "seus" | "complementary" | "vanilla",
	ShaderGrade
> = {
	...
	seus: {
		...
		saturation: 1.06,
		vignette: 0.42,
		shadowTint: [0.8, 0.9, 1.2],
		highTint: [1.0, 1.02, 1.08],
		tintAmount: 0.7,
...
const sunVector = (azimuthDeg: number, elevationDeg: number) => {
	...
	return {
		x: Math.sin(az) * c,
		y: Math.sin(el) * SUN_DISTANCE,
		z: -Math.cos(az) * c,
	};
};

/** そのレイアウトで実際に使われる時間帯。未指定ならシェーダープリセットから決まる。 */
export const yume25dTimeOfDay = (
	L: Pick<Layout25D, "timeOfDay" | "shaderPreset">,
): "day" | "sunset" | "night" =>
	L.timeOfDay ??
	(L.shaderPreset === "seus"
		? "night"
		: L.shaderPreset === "complementary"
			? "day"
			: "sunset");

/** 環境光の明るさの既定値（時間帯ごと）。編集パネルのスライダー初期値もこれに合わせる。 */
export const yume25dAmbientDefault = (
	L: Pick<Layout25D, "timeOfDay" | "shaderPreset">,
): number => TIME_OF_DAY[yume25dTimeOfDay(L)].ambientLight;

/** 実際に使われる太陽（月）の方位角・仰角（度）。未指定なら時間帯ごとの既定値。 */
export const yume25dSunAngles = (
	L: Pick<
		Layout25D,
		"timeOfDay" | "shaderPreset" | "sunAzimuth" | "sunElevation"
	>,
): { azimuth: number; elevation: number } => {
	const look = TIME_OF_DAY[yume25dTimeOfDay(L)];
	return {
		azimuth: L.sunAzimuth ?? look.azimuth,
		elevation: L.sunElevation ?? look.elevation,
	};
};
...
const YAW_FOR_DIR: Record<Dir4, number> = {
	0: 0,
	1: -Math.PI / 2,
	2: Math.PI,
	3: Math.PI / 2,
...
const splitCropUrl = (
	raw: string,
): { url: string; crop: [number, number, number, number] | null } => {
	...
	const parts = raw
		...
		.map(Number);
	const ok =
		parts.length >= 4 && parts.slice(0, 4).every((n) => !Number.isNaN(n));
	return {
		url: raw.slice(0, hashIdx),
		crop: ok ? [parts[0], parts[1], parts[2], parts[3]] : null,
	};
};
...
export const drawPlayerIconCanvas = (
	cv: HTMLCanvasElement,
	a: PlayerAppearance,
	onUpdate?: () => void,
): void => {
...
export class Yume25DEngine {
	...
	constructor(
		...
		this.playerMat = new THREE.MeshLambertMaterial({
	...
	resetToStart() {
		...
		this.vy = 0;
		this.hop = this.groundAt(this.x, this.z, Number.POSITIVE_INFINITY);
		this.grounded = true;
		...
		for (const ab of this.activeBillboards) {
			...
			if (is3DModel) {
				ab.object.rotation.y =
					ab.data.dir !== undefined ? YAW_FOR_DIR[ab.data.dir] : 0;
			}
	...
	setPlayerAppearance(appearance: PlayerAppearance) {
		...
		if (
			p.emoji === appearance.emoji &&
			p.color === appearance.color &&
			p.spriteUrl === appearance.spriteUrl &&
			p.spriteRef === appearance.spriteRef &&
			p.minecraftSkin === appearance.minecraftSkin
		)
			return;
	...
	private applyPlayerAppearance() {
		...
		drawPlayerCanvas(this.playerCanvas, a, () => {
			this.playerTexture.needsUpdate = true;
		});
		this.playerTexture.needsUpdate = true;
	}
	...
	setPov(mode: PovMode, distance?: number) {
		...
		if (distance !== undefined)
			this.povDistance = Math.max(
				POV_MIN_DIST,
				Math.min(POV_MAX_DIST, distance),
			);
		this.playerMesh.visible = this.pov === "third";
	}
	...
	adjustPovDistance(delta: number) {
		...
		this.povDistance = Math.max(
			POV_MIN_DIST,
			Math.min(POV_MAX_DIST, this.povDistance + delta),
		);
	}
	...
	pickTarget(
		...
		if (hit && (preferGeometry || hit.distance <= planeDist)) {
			...
			const p = hit.point
				.clone()
				.addScaledVector(raycaster.ray.direction, -1e-3);
			let level = bbLevel ?? Math.max(0, Math.floor(p.y / H));
			if (
				!preferGeometry &&
				bbLevel !== undefined &&
				hit.face &&
				hit.face.normal.y > 0.5
			) {
				level = bbLevel + 1;
			}
			return { x: p.x, z: p.z, level };
		}
		if (planeHit && planeDist <= this.layout.fogFar)
			return { x: pt.x, z: pt.z, level: flightLevel };
		return null;
	}

	/** 配置プレビュー：ツールが適用される場所へ半透明ゴーストを表示する（null で非表示）。 */
	setGhost(spec: GhostSpec | null) {
		const m = this.ghostMesh,
			mat = this.ghostMat;
		if (!spec) {
			m.visible = false;
			this.ghostKind = null;
		...
		if (spec.kind === "cell") {
			...
			if (spec.kind === "floor") {
				...
				if (spec.dir === 0)
					m.position.set(spec.col + 0.5, y, spec.row); // 北辺
				else {
					m.rotation.y = Math.PI / 2;
					m.position.set(spec.col, y, spec.row + 0.5);
				} // 西辺
			} else {
	...
	private updateSpeeches(nowMs: number) {
		...
		for (const b of this.layout.billboards) {
			...
			const shown = Math.min(
				e.totalChars,
				Math.floor((nowMs - e.start) / SPEECH_CHAR_MS),
			);
			if (shown !== e.shown) {
				e.shown = shown;
				this.drawSpeech(e);
	...
	private createSpeech(b: Billboard25D, nowMs: number): SpeechEntry {
		...
		const geo = new THREE.PlaneGeometry(
			canvas.width / SPEECH_PX_PER_UNIT,
			canvas.height / SPEECH_PX_PER_UNIT,
		);
		...
		mesh.position.set(
			b.col + 0.5,
			topY + SPEECH_MARGIN_Y + canvas.height / SPEECH_PX_PER_UNIT / 2,
			b.row + 0.5,
		);
		...
		if (e.shown > 0) {
			...
			disp.forEach((l, i) =>
				ctx.fillText(
					l,
					cx,
					3 + i * SPEECH_LINE_H + (SPEECH_LINE_H - SPEECH_FONT_PX) / 2,
				),
			);
		}
		...
		const loop = (t: number) => {
			...
			if (this.sunLight) {
				this.sunLight.position.set(
					this.x + this.sunOffset.x,
					this.sunOffset.y,
					this.z + this.sunOffset.z,
				);
	...
	private updateTexAnimations(timeSec: number) {
		for (const entry of this.texEntries.values()) {
			const a = entry.anim;
			if (!a) continue;
			const frame = walkFrameIndex(
				a.std,
				Math.floor(timeSec * BILLBOARD_ANIM_FPS),
			);
	...
	private updateBillboardDirSprites(timeSec: number) {
		const camFx = -Math.sin(this.yaw),
			camFz = -Math.cos(this.yaw);
		const camRx = Math.cos(this.yaw),
			camRz = -Math.sin(this.yaw);
		for (const ab of this.activeBillboards) {
		...
		try {
			...
			if (L.bloomEnabled !== false) {
				...
				const bloomPass = new UnrealBloomPass(
					new THREE.Vector2(RENDER_W, RENDER_H),
					b.strength,
					b.radius,
					b.threshold,
				);
	...
	private buildScene() {
		...
		this.sunOffset = sunVector(
			L.sunAzimuth ?? look.azimuth,
			L.sunElevation ?? look.elevation,
		);
		this.sunLight.visible = !L.sunHidden;
		this.sunLight.castShadow = shadows;
		this.ambientLightObj.color.copy(
			toLightColor(L.ambientColor ?? look.ambient, 0.5),
		);
		this.ambientLightObj.intensity =
			(L.ambientLight ?? look.ambientLight) * AMBIENT_SCALE;
		this.scene.fog = new THREE.Fog(
			new THREE.Color(L.fogColor || look.horizon),
			L.fogNear || 2,
			L.fogFar || 18,
		);
		...
		for (const b of L.billboards) {
				...
				this.loadModel(def.modelUrl).then((res) => {
					...
					if (animations && animations.length > 0) {
						...
						abInstance.anims = {
							...
							run:
								runClip && runClip !== walkClip
									? mixer.clipAction(runClip)
									: undefined,
							idle:
								idleClip && idleClip !== walkClip
						...
						this.applyModelAnim(
							abInstance,
							(b.behavior || "still") === "still" ? "idle" : "walk",
						);
					}
			...
			if (def?.special === "block") {
				...
				if (!blockGeoShared) {
					blockGeoShared = new THREE.BoxGeometry(
						BLOCK_SIZE,
						BLOCK_SIZE,
						BLOCK_SIZE,
					);
	...
	private updateSkyBackground(L: Layout25D, look: TimeOfDayLook) {
		...
		drawSkyCanvas(
			...
			zenith,
			{ ...look, horizon },
			L.sunHidden ? null : this.sunOffset,
		);
	...
	private updateSky(L: Layout25D) {
		...
		if (!this.skyMesh) {
			...
			this.skyGeo = new THREE.CylinderGeometry(
				SKY_RADIUS,
				SKY_RADIUS,
				SKY_HEIGHT,
				24,
	...
	private updateWater(L: Layout25D) {
		...
		if (!this.waterMesh) {
			...
			this.bubbles = Array.from({ length: BUBBLE_MAX }, () => ({
				x: 0,
				y: -1,
				z: 0,
				vy: 0,
			...
			this.bubbleGeo.setAttribute(
				"position",
				new THREE.BufferAttribute(
					new Float32Array(BUBBLE_MAX * 3).fill(-100),
					3,
				),
			);
		...
		if (
			!this.waterGeo ||
			this.waterGeoCols !== L.cols ||
			this.waterGeoRows !== L.rows
		) {
			this.waterGeo?.dispose();
			const segX = Math.min(WAVE_SEG_MAX, L.cols * 2),
				segY = Math.min(WAVE_SEG_MAX, L.rows * 2);
		...
		this.waterMat!.emissive.set(
			lava
				? new THREE.Color(this.waterBaseColor()).multiplyScalar(0.55)
	...
	private waterBaseColor(): string {
		return (
			this.layout.waterColor ??
			((this.layout.waterKind ?? "water") === "lava"
				? LAVA_DEFAULT_COLOR
				: WATER_DEFAULT_COLOR)
		);
	}
	...
	private setUnderwater(on: boolean) {
		...
		if (on) {
			...
			if (this.skyMesh)
				this.skyMesh.visible = !!L.skyUrl && this.skyUrlLoaded === L.skyUrl;
		}
	}
	...
	private updateWaterFX(dt: number, inWater: boolean, waterLv: number) {
		...
		if (inWater && this.playerMoving)
			srcs.push({ x: this.x, z: this.z, amp: 1 });
		for (const ab of this.activeBillboards) {
			if (srcs.length >= WAVE_SRC_MAX) break;
			if (
				(Math.abs(ab.vx) > 0.05 || Math.abs(ab.vz) > 0.05) &&
				ab.y < waterLv
			) {
		...
		if (E < 0.02) {
			...
			const pos = this.waterGeo.attributes.position;
			const cx = this.waterMesh.position.x,
				cz = this.waterMesh.position.z;
			const t = this.waveT;
			for (let i = 0; i < pos.count; i++) {
				...
				let y =
					0.02 * Math.sin(wx * 1.7 + t * 1.6) * Math.sin(wz * 1.4 + t * 1.1);
				for (const s of srcs) {
					const d = Math.hypot(wx - s.x, wz - s.z);
					if (d >= WAVE_RADIUS) continue;
					y +=
						s.amp * 0.035 * (1 - d / WAVE_RADIUS) * Math.sin(d * 5.5 - t * 7);
				}
				pos.setZ(i, y * E);
			}
			pos.needsUpdate = true;
		...
		if (this.bubbleGeo && this.bubblePts?.visible) {
			...
			if (submerged && this.bubbleSpawnT <= 0) {
				this.bubbleSpawnT = this.playerMoving
					? BUBBLE_INTERVAL * 0.5
					: BUBBLE_INTERVAL;
			...
			for (let i = 0; i < this.bubbles.length; i++) {
				...
				arr.setXYZ(
					i,
					b.x + Math.sin(this.waveT * 5 + b.phase) * 0.04,
					b.y,
					b.z,
				);
			}
	...
	private updatePlayObjects(dt: number) {
		...
		if (this.editMode) {
			...
			for (const b of this.balls) {
				...
				if (
					d < PLAYER_RADIUS + b.r &&
					this.hop < this.layout.wallHeight * 0.5 &&
					b.y - b.r < 0.6
				) {
				...
				if (b.vx !== 0 || b.vz !== 0) {
					if (!wasAirborne) {
						...
						b.vx *= decel;
						b.vz *= decel;
						if (Math.hypot(b.vx, b.vz) < BALL_STOP_EPS) {
							b.vx = 0;
							b.vz = 0;
						}
					}
					...
					const by0 = b.y - b.r * 0.6,
						by1 = b.y + b.r * 0.6;
					let nx = b.x + b.vx * dt;
					if (b.vx > 0) {
						const c0 = Math.floor(b.x + b.r),
							c1 = Math.floor(nx + b.r);
						const blocked =
							c1 > c0 &&
							((hitsWall && this.blockedV(c1, b.z)) ||
								this.blockSolidAt(c1, Math.floor(b.z), by0, by1));
						if (blocked || nx > this.layout.cols - b.r) {
							nx = b.x;
					...
					let nz = b.z + b.vz * dt;
					if (b.vz > 0) {
						const r0 = Math.floor(b.z + b.r),
							r1 = Math.floor(nz + b.r);
						const blocked =
							r1 > r0 &&
							((hitsWall && this.blockedH(r1, b.x)) ||
								this.blockSolidAt(Math.floor(b.x), r1, by0, by1));
						if (blocked || nz > this.layout.rows - b.r) {
							nz = b.z;
		...
		for (const s of this.speakers) {
			...
			for (const [px, pz] of s.positions) {
				const dd =
					(px - this.x) * (px - this.x) + (pz - this.z) * (pz - this.z);
				if (dd < d2) d2 = dd;
			}
			const t = Math.max(0, 1 - Math.sqrt(d2) / s.radius);
			const vol = audible
				? Math.min(1, s.volume * t * t) * (applyMasterVolume(100) / 100)
				: 0;
	...
	private resolveBillboardWalls(ab: BillboardInstance) {
		...
		const by0 = ab.y + EPS,
			by1 = ab.y + PLAYER_BODY_H;
		for (let pass = 0; pass < 2; pass++) {
			let moved = false;
			const c0 = Math.ceil(ab.x - r),
				c1 = Math.floor(ab.x + r);
			for (let c = c0; c <= c1; c++) {
				const r0 = Math.floor(ab.z - r * 0.9),
					r1 = Math.floor(ab.z + r * 0.9);
				const pc = ab.x < c ? c : c - 1; // x=c の辺を越えて入り込むセルの列
				let blocked = false;
				for (let row = r0; row <= r1; row++) {
					if (
						this.vEdges.has(`${c},${row}`) ||
						this.blockSolidAt(pc, row, by0, by1)
					) {
						blocked = true;
						break;
					}
			...
			const r0 = Math.ceil(ab.z - r),
				r1 = Math.floor(ab.z + r);
			for (let rr = r0; rr <= r1; rr++) {
				const c0_v = Math.floor(ab.x - r * 0.9),
					c1_v = Math.floor(ab.x + r * 0.9);
				const pr = ab.z < rr ? rr : rr - 1; // z=rr の辺を越えて入り込むセルの行
				let blocked = false;
				for (let col = c0_v; col <= c1_v; col++) {
					if (
						this.hEdges.has(`${col},${rr}`) ||
						this.blockSolidAt(col, pr, by0, by1)
					) {
						blocked = true;
						break;
					}
		...
		ab.x = Math.max(
			r_bound + EPS,
			Math.min(this.layout.cols - r_bound - EPS, ab.x),
		);
		ab.z = Math.max(
			r_bound + EPS,
			Math.min(this.layout.rows - r_bound - EPS, ab.z),
		);
	...
	private applyModelAnim(
		ab: BillboardInstance,
		mode: "idle" | "walk" | "run",
		timeScale = 1,
	) {
		...
		if (mode === "idle" && !anims.idle) {
			if (ab.currentAction && ab.currentAction !== anims.walk)
				ab.currentAction.stop();
			anims.walk.play();
	...
	private updateBillboardAI(dt: number) {
		...
		for (const ab of this.activeBillboards) {
			...
			if (
				...
				!this.demo &&
				wd &&
				(behavior === "chase" ? wd.enemy : wd.npc) &&
				ab.y < waterLv - 1e-3
			) {
				ab.submergeHp =
					(ab.submergeHp ?? NPC_SUBMERGE_SEC) - dt * (lavaSea ? 2 : 1);
				if (ab.submergeHp <= 0) {
					ab.dead = true;
				...
				ab.object.visible =
					ab.submergeHp > 1.0 || Math.floor(ab.submergeHp * 8) % 2 === 0;
			} else if (
				ab.submergeHp !== undefined &&
				ab.submergeHp < NPC_SUBMERGE_SEC
			) {
			...
			if (behavior === "patrolH" && Math.abs(ab.x - prevX) < 1e-4) {
				...
				(behavior === "walker" || behavior === "randomHop") &&
				Math.hypot(ab.x - prevX, ab.z - prevZ) < 1e-4 * speed
			) {
				const theta = Math.random() * Math.PI * 2;
				ab.vx = Math.cos(theta) * speed;
				ab.vz = Math.sin(theta) * speed;
			...
			if (sp > 1e-4) {
				if (sp > speed * 1.5) this.applyModelAnim(ab, "run");
					...
					this.applyModelAnim(
						ab,
						"walk",
						Math.min(1.6, Math.max(0.75, sp / speed)),
					);
			} else {
				this.applyModelAnim(ab, "idle");
			}
			...
			if (ab.mcLimbs) {
				...
				if (sp > 1e-4) {
					...
					for (const limb of [L.rArm, L.lArm, L.rLeg, L.lLeg])
						limb.rotation.x *= k;
				}
			}
	...
	private clampToBounds() {
		this.x = Math.max(
			PLAYER_RADIUS + EPS,
			Math.min(this.layout.cols - PLAYER_RADIUS - EPS, this.x),
		);
		this.z = Math.max(
			PLAYER_RADIUS + EPS,
			Math.min(this.layout.rows - PLAYER_RADIUS - EPS, this.z),
		);
	}
	...
	private blockedV(c: number, z: number): boolean {
		...
		const r0 = Math.floor(z - PLAYER_RADIUS * 0.9),
			r1 = Math.floor(z + PLAYER_RADIUS * 0.9);
		for (let r = r0; r <= r1; r++)
			if (this.vEdges.has(`${c},${r}`)) return true;
		return false;
	}
	/** z=r の横辺が、プレイヤーの x 範囲のどこかで壁になっているか。 */
	private blockedH(r: number, x: number): boolean {
		if (this.aboveWalls()) return false;
		const c0 = Math.floor(x - PLAYER_RADIUS * 0.9),
			c1 = Math.floor(x + PLAYER_RADIUS * 0.9);
		for (let c = c0; c <= c1; c++)
			if (this.hEdges.has(`${c},${r}`)) return true;
		return false;
	}
	...
	private resolveWalls() {
		...
		for (let pass = 0; pass < 2; pass++) {
			let moved = false;
			const c0 = Math.ceil(this.x - r),
				c1 = Math.floor(this.x + r);
			for (let c = c0; c <= c1; c++) {
				if (!this.blockedV(c, this.z)) continue;
				this.x = this.x < c ? c - r - EPS : c + r + EPS;
				moved = true;
			}
			const r0 = Math.ceil(this.z - r),
				r1 = Math.floor(this.z + r);
			for (let rr = r0; rr <= r1; rr++) {
	...
	private blockedByBlockX(c: number, z: number): boolean {
		const r0 = Math.floor(z - PLAYER_RADIUS * 0.9),
			r1 = Math.floor(z + PLAYER_RADIUS * 0.9);
		for (let r = r0; r <= r1; r++)
			if (this.blockedByBlockCell(c, r)) return true;
		return false;
	}
	...
	private moveX(dx: number) {
		...
		let nx = this.x + dx;
		if (dx > 0) {
			const b = Math.floor(this.x + PLAYER_RADIUS),
				nb = Math.floor(nx + PLAYER_RADIUS);
			if (
				nb > b &&
				(this.blockedV(nb, this.z) || this.blockedByBlockX(nb, this.z))
			)
				nx = nb - PLAYER_RADIUS - EPS;
		} else {
			const b = Math.floor(this.x - PLAYER_RADIUS),
				nb = Math.floor(nx - PLAYER_RADIUS);
			if (
				nb < b &&
				(this.blockedV(b, this.z) || this.blockedByBlockX(nb, this.z))
			)
	...
	private moveZ(dz: number) {
		...
		let nz = this.z + dz;
		if (dz > 0) {
			const b = Math.floor(this.z + PLAYER_RADIUS),
				nb = Math.floor(nz + PLAYER_RADIUS);
			if (
				nb > b &&
				(this.blockedH(nb, this.x) || this.blockedByBlockZ(nb, this.x))
			)
				nz = nb - PLAYER_RADIUS - EPS;
		} else {
			const b = Math.floor(this.z - PLAYER_RADIUS),
				nb = Math.floor(nz - PLAYER_RADIUS);
			if (
				nb < b &&
				(this.blockedH(b, this.x) || this.blockedByBlockZ(nb, this.x))
			)
	...
	private raycastClamp(backX: number, backZ: number, maxDist: number): number {
		...
		let limit = maxDist;
		if (backX > 1e-6)
			limit = Math.min(limit, (this.layout.cols - MARGIN - this.x) / backX);
		else if (backX < -1e-6) limit = Math.min(limit, (MARGIN - this.x) / backX);
		if (backZ > 1e-6)
			limit = Math.min(limit, (this.layout.rows - MARGIN - this.z) / backZ);
		...
		for (let i = 1; i <= STEPS; i++) {
			...
			const nx = this.x + backX * t,
				nz = this.z + backZ * t;
			const bx0 = Math.floor(px),
				bx1 = Math.floor(nx);
			if (
				bx1 !== bx0 &&
				this.vEdges.has(`${Math.max(bx0, bx1)},${Math.floor(nz)}`)
			) {
				// 交差した縦辺（x=整数）までの正確な距離
				const tHit =
					Math.abs(backX) > 1e-6
						? (Math.max(bx0, bx1) - this.x) / backX
						: (limit * (i - 1)) / STEPS;
				return Math.max(POV_MIN_DIST, Math.min(limit, tHit - MARGIN));
			}
			const bz0 = Math.floor(pz),
				bz1 = Math.floor(nz);
			if (
				bz1 !== bz0 &&
				this.hEdges.has(`${Math.floor(nx)},${Math.max(bz0, bz1)}`)
			) {
				const tHit =
					Math.abs(backZ) > 1e-6
						? (Math.max(bz0, bz1) - this.z) / backZ
						: (limit * (i - 1)) / STEPS;
				return Math.max(POV_MIN_DIST, Math.min(limit, tHit - MARGIN));
			}
			px = nx;
	...
	private floorTexAt(x: number, z: number): Tex25D | undefined {
		const c = Math.floor(x),
			r = Math.floor(z);
		if (c < 0 || r < 0 || c >= this.layout.cols || r >= this.layout.rows)
			return undefined;
	...
	private step(dt: number) {
		...
		if (specActive) {
			...
			if (this.iceBlockedCell && this.iceBlockedCell !== cellKey)
				this.iceBlockedCell = null;
			...
			if (vec && this.iceBlockedCell !== cellKey) {
				...
				this.clampToBounds();
				this.resolveWalls();
				if (Math.hypot(this.x - sx, this.z - sz) < d * 0.25)
					this.iceBlockedCell = cellKey;
				else slideVec = vec;
			}
		} else {
			this.iceBlockedCell = null;
		...
		if (!slideVec && (move !== 0 || strafe !== 0)) {
			...
			// ダッシュ移動中は空腹がごくゆっくり減る
			if (dashMult > 1 && this.layout.hunger && !this.editMode)
				this.setHunger(this.hunger - HUNGER_DASH_DRAIN * dt);
			const swimMult = inWater ? WATER_MOVE_MULT : 1; // 水中は泳ぎでゆっくり進む
			const ms = move * MOVE_SPEED * dashMult * swimMult * dt,
				ss = strafe * STRAFE_SPEED * dashMult * swimMult * dt;
		...
		if (slideVec) {
			...
			const df = slideVec[0] * fx + slideVec[1] * fz,
				dr = slideVec[0] * rx + slideVec[1] * rz;
			this.playerDir =
		...
		if (this.demo && move !== 0 && this.demoTurnFrames <= 0) {
			...
			if (moved < expected * 0.35)
				this.demoTurnFrames = 25 + Math.floor(Math.random() * 50);
		}
		...
		if (hovering) {
			...
			if (fly !== 0) {
				this.hop = Math.max(
					0,
					Math.min(FLY_MAX_ALT, this.hop + fly * FLY_SPEED * dt),
				);
				if (this.hop < H) this.resolveWalls(); // 壁の高さより下へ降りたら壁の中に居座らないよう押し出す
			}
			this.vy = 0;
			this.grounded =
				this.hop <= this.groundAt(this.x, this.z, this.hop) + 1e-3;
			this.jumpQueued = false;
		} else {
			...
			if (this.jumpQueued && (this.grounded || inWater)) {
				this.vy = inWater
					? SWIM_UP_V
					: (this.layout.jumpHeight ?? JUMP_VELOCITY_DEFAULT);
				this.grounded = false;
				this.jumpQueued = false;
			}
			// 足場＝地面0＋ブロック上面。
			...
			const ground = this.groundAt(
				this.x,
				this.z,
				this.hop + (this.grounded ? BLOCK_CLIMB_MAX : 0),
			);
			if (this.grounded && this.hop < ground)
				this.hop = Math.min(ground, this.hop + CLIMB_SPEED * dt); // 段差をよいしょと上がる
		...
		if (this.playerMcGroup) {
			...
			if (isMoving && this.grounded) {
				...
				const k = Math.max(0, 1 - dt * 8);
				for (const limb of [
					this.playerMcLimbs.rArm,
					this.playerMcLimbs.lArm,
					this.playerMcLimbs.rLeg,
					this.playerMcLimbs.lLeg,
				])
					limb.rotation.x *= k;
			}
		} else {
			...
			this.playerMesh.position.set(
				this.x,
				this.hop + 0.42 * this.playerScale,
				this.z,
			);
		...
		if (this.pov === "third") {
			...
			this.camera.rotation.x =
				-Math.atan2(above + 0.1, Math.max(0.35, dist)) + this.pitch;
		} else {
		...
		if (
			playActive &&
			!this.fadeState &&
			this.layout.waterDamage?.player &&
			inWater
		) {
		...
		if (this.layout.oxygen) {
			...
			if (playActive && headUnder) {
				...
				this.oxygen = Math.min(
					OXYGEN_MAX_SEC,
					this.oxygen + dt * OXYGEN_RECOVER_MULT,
				);
				this.drownT = 0;
			}
		...
		if (this.layout.hunger && playActive) {
			...
			if (this.hunger < HUNGER_MAX - 1e-3) {
				for (const ab of this.activeBillboards) {
					...
					if (
						Math.abs(ab.y - this.hop) > 1.0 ||
						Math.hypot(ab.x - this.x, ab.z - this.z) > 0.55
					)
						continue;
					ab.dead = true;
					...
					for (let i = 0; i < 3; i++)
						setTimeout(() => playSysSfx(SYS_FOOD_SFX), i * 260);
					break; // 1フレームに食べるのは1個
				}
			}
		}
