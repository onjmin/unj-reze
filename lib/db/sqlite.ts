...
function ensureTableMigrations(d: SqlJsDatabase) {
	...
	if (!colNames.includes("type")) {
		d.run(
			"ALTER TABLE notifications ADD COLUMN type TEXT NOT NULL DEFAULT 'like'",
		);
	}
	...
	const userColNames =
		userCols.length > 0 ? userCols[0].values.map((v: any) => v[1]) : [];
	...
	const msgColNames =
		msgCols.length > 0 ? msgCols[0].values.map((v: any) => v[1]) : [];
	...
	const gameColNames =
		gameCols.length > 0 ? gameCols[0].values.map((v: any) => v[1]) : [];
	...
	const mvColNames =
		mvCols.length > 0 ? mvCols[0].values.map((v: any) => v[1]) : [];
	...
	const postColNames =
		postCols.length > 0 ? postCols[0].values.map((v: any) => v[1]) : [];
	...
	if (!postColNames.includes("is_false_declaration")) {
		d.run(
			"ALTER TABLE posts ADD COLUMN is_false_declaration INTEGER NOT NULL DEFAULT 0",
		);
	}
	...
	if (!postColNames.includes("has_mml")) {
		...
		d.run(
			"UPDATE posts SET has_mml = 1 WHERE content LIKE '%#mml%' OR content LIKE '%#MML作曲%'",
		);
	}
	...
	const auColNames =
		auCols.length > 0 ? auCols[0].values.map((v: any) => v[1]) : [];
	if (!auColNames.includes("is_private"))
		d.run(
			"ALTER TABLE anonymous_users ADD COLUMN is_private INTEGER NOT NULL DEFAULT 0",
		);
	if (!auColNames.includes("hide_from_search"))
		d.run(
			"ALTER TABLE anonymous_users ADD COLUMN hide_from_search INTEGER NOT NULL DEFAULT 0",
		);
	if (!auColNames.includes("hide_reactions"))
		d.run(
			"ALTER TABLE anonymous_users ADD COLUMN hide_reactions INTEGER NOT NULL DEFAULT 0",
		);
	...
	const oshiColNames =
		oshiCols.length > 0 ? oshiCols[0].values.map((v: any) => v[1]) : [];
	if (!oshiColNames.includes("preview_url"))
		d.run("ALTER TABLE oshi_items ADD COLUMN preview_url TEXT");
}
...
/** 通知を挿入。自己宛は生成しない。 */
function insertNotificationSqlite(
	d: SqlJsDatabase,
	data: { recipientId: string; actor: string; type: string; postId?: number },
): void {
	...
	try {
       ...
       VALUES (?, ?, ?, ?, ?, 0, ?)`,
		...
		/* notifications 未整備時は無視 */
	}
}

/** userId(匿名ID/displayName/slug) から slug を解決。 */
function resolveViewerSlugSqlite(d: SqlJsDatabase, userId: string): string {
	const u = rowsToObjects(
		d,
		"SELECT slug FROM anonymous_users WHERE id = ? OR display_name = ? OR slug = ? LIMIT 1",
...
function getHiddenSlugsSqlite(d: SqlJsDatabase, userId?: string): Set<string> {
	...
	const blocks = rowsToObjects(
		d,
		"SELECT blocker_slug, blocked_slug FROM user_blocks WHERE blocker_slug = ? OR blocked_slug = ?",
	...
	const mutes = rowsToObjects(
		d,
		"SELECT muted_slug FROM user_mutes WHERE muter_slug = ?",
		[viewerSlug],
	);
...
function randomGradientSqlite(): string {
	return AVATAR_GRADIENTS_SQLITE[
		Math.floor(Math.random() * AVATAR_GRADIENTS_SQLITE.length)
	];
}
...
async function getThreadRepliesSqlite(
	d: SqlJsDatabase,
	threadIds: number[],
...
export const sqliteStore: DataStore = {
	async getPosts(
		...
		return rows
			.map((r) => ({
				...rowToPost(r),
				replies: (repliesMap.get(r.id) || []).filter(
					(rep) => !hidden.has(rep.slug ?? ""),
				),
			}))
	...
	async getPost(id: number, userId?: string) {
		...
		if (userId) {
			...
			rows = rowsToObjects(d, `${UNVOTED_SELECT} WHERE p.id = ?`, [id]);
		}
	...
	async createPost(data: CreatePostParams) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			[
				...
				hasMml,
				data.mmlUrl || null,
				data.mmlDeleteId || null,
				data.mmlDeleteHash || null,
				originTypeVal,
			],
		);
		saveDb();
		return {
			...rowToPost({
				...
				has_collab_button: 1,
				hearts_total: 0,
				has_game: data.gameId ? 1 : 0,
				game_id: data.gameId || null,
				has_mv: data.mvId ? 1 : 0,
				mv_id: data.mvId || null,
				has_mml: hasMml,
				mml_url: data.mmlUrl || null,
	...
	async likePost(id: number, userId: string) {
		...
		if (existingVote === "like") {
			d.run("DELETE FROM post_votes WHERE post_id = ? AND user_id = ?", [
				id,
				userId,
			]);
			...
			d.run(
				"UPDATE posts SET likes = likes + 1, dislikes = MAX(dislikes - 1, 0) WHERE id = ?",
				[id],
			);
		} else {
			d.run(
			...
			const authorRows = rowsToObjects(
				d,
				"SELECT slug FROM posts WHERE id = ?",
				[id],
			);
			if (authorRows.length > 0) {
				insertNotificationSqlite(d, {
					recipientId: authorRows[0].slug,
					actor: userId,
					type: "like",
					postId: id,
				});
		...
		const rows = rowsToObjects(d, `${VOTED_SELECT} WHERE p.id = ?`, [
			userId,
			id,
		]);
		if (rows.length === 0) return null;
	...
	async dislikePost(id: number, userId: string) {
		...
		if (existingVote === "dislike") {
			...
			d.run("UPDATE posts SET dislikes = MAX(dislikes - 1, 0) WHERE id = ?", [
				id,
			]);
			...
			d.run(
				"UPDATE posts SET dislikes = dislikes + 1, likes = MAX(likes - 1, 0) WHERE id = ?",
				[id],
			);
		} else {
			d.run(
	...
	async heartPost(id: number, userId: string, count: number = 1) {
		const d = await getDb();
		const postRows = rowsToObjects(d, "SELECT id FROM posts WHERE id = ?", [
			id,
		]);
		...
		for (let i = 0; i < n; i++) {
			d.run("INSERT INTO post_hearts (post_id, user_id) VALUES (?, ?)", [
				id,
				userId,
			]);
		}
		d.run(
			"UPDATE posts SET hearts_total = COALESCE(hearts_total, 0) + ? WHERE id = ?",
			[n, id],
		);
		const heartAuthor = rowsToObjects(
			d,
			"SELECT slug FROM posts WHERE id = ?",
			[id],
		);
		if (heartAuthor.length > 0) {
			insertNotificationSqlite(d, {
				recipientId: heartAuthor[0].slug,
				actor: userId,
				type: "heart",
				postId: id,
			});
	...
	async addReply(postId: number, data: ReplyParams) {
		...
		const threadRows = rowsToObjects(
			d,
			"SELECT replies_count FROM posts WHERE id = ?",
			[postId],
		);
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		d.run("UPDATE posts SET replies_count = replies_count + 1 WHERE id = ?", [
			postId,
		]);
		const parentRows = rowsToObjects(d, "SELECT slug FROM posts WHERE id = ?", [
			parentPostId,
		]);
	...
	async editPost(
		...
		const rows = rowsToObjects(
			d,
			"SELECT slug, display_name, content, origin_type FROM posts WHERE id = ?",
			[id],
		);
		if (rows.length === 0) return null;
		const viewerSlug = resolveViewerSlugSqlite(d, userId);
		if (rows[0].slug !== viewerSlug) return null;

		const hasContentChanged = rows[0].content !== content;
		const hasOriginTypeChanged =
			originType !== undefined && rows[0].origin_type !== (originType ?? null);
		...
		saveDb();
		const updated = rowsToObjects(d, `${VOTED_SELECT} WHERE p.id = ?`, [
			userId,
			id,
		]);
	...
	async deletePost(id: number, userId: string) {
		...
		const childRows = rowsToObjects(
			d,
			"SELECT COUNT(*) AS cnt FROM posts WHERE thread_id = ? AND id != thread_id",
			[id],
		);
		...
		if (!isReply && hasChildren) {
			d.run(
				`UPDATE posts SET content = '(削除されました)', has_image = 0, image_src = NULL, has_game = 0, game_id = NULL, has_mv = 0, mv_id = NULL WHERE id = ?`,
				[id],
			);
		} else {
			...
			if (isReply)
				d.run(
					"UPDATE posts SET replies_count = MAX(replies_count - 1, 0) WHERE id = ?",
					[post.thread_id],
				);
		}
		saveDb();
	...
	async deleteMessage(id: number, userId: string) {
		const d = await getDb();
		const rows = rowsToObjects(d, "SELECT sender FROM messages WHERE id = ?", [
			id,
		]);
		if (rows.length === 0) return false;
		const sender = rows[0].sender;
		const viewerSlug = resolveViewerSlugSqlite(d, userId);
		if (deriveSlugSqlite(sender) !== viewerSlug) return false;
	...
	async getUserDisplayName(slug: string) {
		...
		const userRows = rowsToObjects(
			d,
			"SELECT display_name FROM anonymous_users WHERE slug = ? LIMIT 1",
			[slug],
		);
		if (userRows.length > 0) return userRows[0].display_name;
		const rows = rowsToObjects(
			d,
			"SELECT display_name FROM posts WHERE slug = ? LIMIT 1",
			[slug],
		);
	...
	async getUserAvatarUrl(slug: string) {
		...
		const rows = rowsToObjects(
			d,
			"SELECT avatar_url FROM anonymous_users WHERE slug = ? LIMIT 1",
			[slug],
		);
	...
	async getUserBio(slug: string) {
		...
		const rows = rowsToObjects(
			d,
			"SELECT bio FROM anonymous_users WHERE slug = ? LIMIT 1",
			[slug],
		);
	...
	async listOshiItems(userSlug: string) {
		...
		const rows = rowsToObjects(
			d,
			"SELECT * FROM oshi_items WHERE user_slug = ? ORDER BY position ASC, id ASC",
			[userSlug],
		);
	...
	async addOshiItem(userSlug: string, data) {
		const d = await getDb();
		const id = Date.now() + Math.floor(Math.random() * 1000);
		const posRows = rowsToObjects(
			d,
			"SELECT COALESCE(MAX(position), -1) + 1 AS next_pos FROM oshi_items WHERE user_slug = ?",
			[userSlug],
		);
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		saveDb();
		const rows = rowsToObjects(d, "SELECT * FROM oshi_items WHERE id = ?", [
			id,
		]);
	...
	async removeOshiItem(userSlug: string, id: number) {
		const d = await getDb();
		d.run("DELETE FROM oshi_items WHERE id = ? AND user_slug = ?", [
			id,
			userSlug,
		]);
		saveDb();
	...
	async markNotificationRead(id: number, userId: string) {
		...
		const slug = resolveViewerSlugSqlite(d, userId);
		d.run(
			"UPDATE notifications SET read = 1 WHERE id = ? AND target_slug = ?",
			[id, slug],
		);
		saveDb();
	},
	...
	async markAllNotificationsRead(userId: string) {
		...
		const slug = resolveViewerSlugSqlite(d, userId);
		d.run(
			"UPDATE notifications SET read = 1 WHERE target_slug = ? AND read = 0",
			[slug],
		);
		saveDb();
	},
	...
	async deleteNotification(id: number, userId: string) {
		const d = await getDb();
		const slug = resolveViewerSlugSqlite(d, userId);
		d.run("DELETE FROM notifications WHERE id = ? AND target_slug = ?", [
			id,
			slug,
		]);
		saveDb();
	...
	async getUnreadCount(userId: string) {
		...
		const rows = rowsToObjects(
			d,
			"SELECT COUNT(*) AS cnt FROM notifications WHERE target_slug = ? AND read = 0",
			[slug],
		);
		return rows[0]?.cnt ?? 0;
	},
	...
	async addMessage(data: MessageParams) {
		...
		const recipientSlug = data.recipient
			? resolveViewerSlugSqlite(d, data.recipient)
			: null;
		d.run(
		...
		return {
			...
			sender: senderSlug,
			text: data.text,
			recipient: recipientSlug ?? undefined,
			createdAt: now,
	...
	async getAnonymousUserBySession(
		sessionId: string,
	): Promise<AnonymousUser | null> {
		const d = await getDb();
		const rows = rowsToObjects(
			d,
			"SELECT * FROM anonymous_users WHERE session_id = ? LIMIT 1",
			[sessionId],
		);
		const row = rows[0];
		if (!row) return null;
	...
	async getOrCreateAnonymousUser(sessionId: string, ipAddress: string) {
		...
		if (sessionRows.length > 0) {
			...
			d.run("UPDATE anonymous_users SET last_seen_at = ? WHERE id = ?", [
				new Date().toISOString(),
				row.id,
			]);
			saveDb();
	...
	async updateUserDisplayName(
		...
		const d = await getDb();
		let userRows = rowsToObjects(
			d,
			"SELECT id, slug FROM anonymous_users WHERE id = ?",
			[userId],
		);
		if (userRows.length === 0) {
			userRows = rowsToObjects(
				d,
				"SELECT id, slug FROM anonymous_users WHERE slug = ?",
				[userId],
			);
		}
		if (userRows.length === 0) {
			userRows = rowsToObjects(
				d,
				"SELECT id, slug FROM anonymous_users WHERE display_name = ?",
				[userId],
			);
		}
		if (userRows.length === 0) {
		...
		if (displayName !== undefined && slug) {
			d.run("UPDATE posts SET display_name = ? WHERE slug = ?", [
				displayName,
				slug,
			]);
		}
	...
	async getUserSettings(slug: string) {
		...
		const rows = rowsToObjects(
			d,
			"SELECT is_private, hide_from_search, hide_reactions FROM anonymous_users WHERE slug = ? LIMIT 1",
			[slug],
		);
		const row = rows[0];
		return {
	...
	async updateUserSettings(
		...
		if (settings.isPrivate !== undefined) {
			...
			vals.push(settings.isPrivate ? 1 : 0);
		}
		if (settings.hideFromSearch !== undefined) {
			sets.push("hide_from_search = ?");
			vals.push(settings.hideFromSearch ? 1 : 0);
		}
		if (settings.hideReactions !== undefined) {
			sets.push("hide_reactions = ?");
			vals.push(settings.hideReactions ? 1 : 0);
		}
		if (sets.length === 0) return;
		vals.push(slug);
	...
	async issueMigrationToken(userId: string) {
		...
		d.run("INSERT INTO migration_tokens (token, user_id) VALUES (?, ?)", [
			token,
			userId,
		]);
		saveDb();
	...
	async redeemMigrationToken(token: string, newSessionId: string) {
		const d = await getDb();
		const tr = rowsToObjects(
			d,
			"SELECT user_id FROM migration_tokens WHERE token = ?",
			[token],
		);
		if (tr.length === 0) return null;
		const userId = tr[0].user_id;
		const ur = rowsToObjects(d, "SELECT * FROM anonymous_users WHERE id = ?", [
			userId,
		]);
		if (ur.length === 0) return null;
		d.run(
			"UPDATE anonymous_users SET session_id = ?, last_seen_at = ? WHERE id = ?",
			[newSessionId, new Date().toISOString(), userId],
		);
	...
	async getFollowCounts(userId: string) {
		const d = await getDb();
		const slug = resolveViewerSlugSqlite(d, userId);
		const followers = rowsToObjects(
			d,
			"SELECT COUNT(*) AS cnt FROM user_follows WHERE followed_id = ? OR followed_id = ?",
			[slug, userId],
		);
		const following = rowsToObjects(
			d,
			"SELECT COUNT(*) AS cnt FROM user_follows WHERE follower_id = ? OR follower_id = ?",
			[slug, userId],
		);
		return {
	...
	async blockUser(blockerSlug: string, blockedSlug: string) {
		...
		const d = await getDb();
		d.run(
			"INSERT OR IGNORE INTO user_blocks (blocker_slug, blocked_slug) VALUES (?, ?)",
	...
	async unblockUser(blockerSlug: string, blockedSlug: string) {
		const d = await getDb();
		d.run(
			"DELETE FROM user_blocks WHERE blocker_slug = ? AND blocked_slug = ?",
			[blockerSlug, blockedSlug],
		);
		saveDb();
	},
	...
	async getBlockedSlugs(blockerSlug: string) {
		...
		return rowsToObjects(
			d,
			"SELECT blocked_slug FROM user_blocks WHERE blocker_slug = ?",
			[blockerSlug],
		).map((r: any) => r.blocked_slug);
	},
	...
	async muteUser(muterSlug: string, mutedSlug: string) {
		...
		const d = await getDb();
		d.run(
			"INSERT OR IGNORE INTO user_mutes (muter_slug, muted_slug) VALUES (?, ?)",
			[muterSlug, mutedSlug],
		);
		saveDb();
	},
	...
	async unmuteUser(muterSlug: string, mutedSlug: string) {
		const d = await getDb();
		d.run("DELETE FROM user_mutes WHERE muter_slug = ? AND muted_slug = ?", [
			muterSlug,
			mutedSlug,
		]);
		saveDb();
	...
	async getMutedSlugs(muterSlug: string) {
		...
		return rowsToObjects(
			...
			"SELECT muted_slug FROM user_mutes WHERE muter_slug = ?",
			[muterSlug],
		).map((r: any) => r.muted_slug);
	},

	async reportContent(data: ReportParams) {
		const d = await getDb();
		d.run(
			"INSERT INTO reports (reporter_slug, target_type, target_id, reason) VALUES (?, ?, ?, ?)",
	...
	async getPostIdByGameId(gameId: number) {
		...
		const rows = rowsToObjects(
			d,
			"SELECT id FROM posts WHERE game_id = ? ORDER BY id ASC LIMIT 1",
			[gameId],
		);
	...
	async getGamesByIds(ids) {
		...
		const rows = rowsToObjects(
			d,
			`SELECT id, preset, title, manifest_url, bg_ref, created_at, creator_slug, plays, clears, best_score, best_score_by FROM games WHERE id IN (${placeholders})`,
			ids,
		);
	...
	async listAllGames(limit?: number) {
		...
		const rows = rowsToObjects(
			d,
			`SELECT * FROM games ORDER BY id DESC LIMIT ${safeLimit}`,
			[],
		);
		return rows.map(rowToGame);
	},
	...
	async getLiveGameInfo(ipAddress: string) {
		...
		const schedRows = rowsToObjects(
			d,
			"SELECT game_id FROM game_schedule WHERE hour_slot = ?",
			[slot],
		);
		...
		if (schedRows.length > 0) {
			...
			const lastSlot = new Date(Date.now() - 3600000)
				.toISOString()
				.slice(0, 13);
			const voteRows = rowsToObjects(
				d,
				"SELECT game_id, COUNT(*) as cnt FROM game_votes WHERE hour_slot = ? GROUP BY game_id ORDER BY cnt DESC LIMIT 1",
				[lastSlot],
			);
			if (voteRows.length > 0) {
				gameId = voteRows[0].game_id;
			} else {
				const allGames = rowsToObjects(
					d,
			...
			if (gameId) {
				d.run(
					"INSERT OR IGNORE INTO game_schedule (hour_slot, game_id) VALUES (?, ?)",
					[slot, gameId],
				);
				saveDb();
			}
		...
		let gameTitle = "",
			gamePreset = "";
		if (gameId) {
			const gr = rowsToObjects(
				d,
				"SELECT preset, title FROM games WHERE id = ?",
				[gameId],
			);
			if (gr.length > 0) {
				gameTitle = gr[0].title;
				gamePreset = gr[0].preset;
			}
		}
		const allGames = rowsToObjects(
			d,
		...
		const vcRows = rowsToObjects(
			d,
			"SELECT game_id, COUNT(*) as cnt FROM game_votes WHERE hour_slot = ? GROUP BY game_id",
			[slot],
		);
		const voteCounts = new Map(
			vcRows.map((r: any) => [r.game_id, Number(r.cnt)]),
		);
		const myVoteRows = rowsToObjects(
			d,
			"SELECT game_id FROM game_votes WHERE ip_address = ? AND hour_slot = ?",
			[ipAddress, slot],
		);
		const myVote = myVoteRows.length > 0 ? myVoteRows[0].game_id : null;
		const nextCandidates = allGames
			.map((g: any) => ({
				game: {
					id: g.id,
					preset: g.preset,
	...
	async voteGame(gameId: number, ipAddress: string) {
		...
		const slot = new Date().toISOString().slice(0, 13);
		d.run(
			"INSERT OR REPLACE INTO game_votes (game_id, ip_address, hour_slot) VALUES (?, ?, ?)",
