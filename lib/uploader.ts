...
const UPLOAD_SECRET_PEPPER = process.env.NEXT_PUBLIC_UPLOADER_UPLOAD_SECRET_PEPPER || '';
 ...
 */
export async function uploadText(kind: UploadKind, text: string): Promise<UploadResult> {
  ...
  if (!res.ok) throw new Error(`アップロードに失敗しました: ${res.status} ${await res.text()}`);
...
export async function uploadJson(kind: 'mv' | 'game', value: unknown): Promise<UploadResult> {
 ...
 * 失敗した時点で投稿が復旧不能になる。
 */
export async function deleteObject(deleteId: string, deleteHash: string): Promise<void> {
  if (!isUploaderAvailable) return;
  const params = new URLSearchParams({ delete_id: deleteId, delete_hash: deleteHash });
