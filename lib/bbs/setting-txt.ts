...
/**
 * 専ブラ対応: SETTING.TXT の中身。仕様: https://info.5ch.io/index.php/SETTING.TXT
 * 専ブラはスレ一覧取得前にこれを読み、書き込み制限(文字数上限等)や名無し名を知る。
 * 未対応だと「無題」表示や文字数制限0扱いなど専ブラ側で誤動作しやすい。
 *
 * 本来の置き場所は板ルート（app/unj/SETTING.TXT）だが、サイトルート直下でないと
 * 認識しない専ブラも存在するため app/SETTING.TXT からも同じ内容を返す
 * （どちらも本ファイルを呼ぶだけで、内容の二重管理はしない）。
 *
 * キーの意味(使うものだけ):
 *   BBS_TITLE / BBS_TITLE_ORIG  板名(表示用/検索用。同じ値でよい)
 *   BBS_NONAME_NAME             名前欄を空で投稿したときのデフォルト名
 *   BBS_SUBJECT_COUNT           subject.txt に載せるスレ数上限(app/unj/subject.txt の MAX_THREADS と揃える)
 ...
 */
export function settingTxtResponse() {
	const lines = [
		`BBS_TITLE=${SITE_NAME}`,
