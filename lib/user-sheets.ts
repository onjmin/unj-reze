...
function read(): UserSheet[] {
	...
	try {
		...
		cache = Array.isArray(arr)
			? arr.filter(
					(s) => s && s.url && (s.pickRef || (s.cellW > 0 && s.cellH > 0)),
				)
			: [];
	} catch {
		cache = [];
...
export function updateUserSheet(
	id: string,
	patch: Partial<Omit<UserSheet, "id">>,
): void {
...
export function exportUserSheet(sheet: UserSheet): string {
	const payload: SheetExport = {
		...
		sheet: {
			...
			cellW: sheet.cellW,
			cellH: sheet.cellH,
			...(sheet.pickRef ? { pickRef: sheet.pickRef } : {}),
		},
	};
...
export function importUserSheet(json: string): UserSheet | null {
	...
	const pickRef =
		typeof s.pickRef === "string" && s.pickRef.trim()
			? s.pickRef.trim()
			: undefined;
	const cellW = Number(s.cellW),
		cellH = Number(s.cellH);
...
export function userSheetCellRef(
	sheet: UserSheet,
	col: number,
	row: number,
): string {
...
export function userSheetCellUrl(
	sheet: UserSheet,
	col: number,
	row: number,
): string {
