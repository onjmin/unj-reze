...
export function usePostActions(
	...
	const heartTimers = useRef<Map<string, ReturnType<typeof setTimeout>>>(
		new Map(),
	);
	const likeParity = useRef<Map<string, number>>(new Map());
	const likeTimers = useRef<Map<string, ReturnType<typeof setTimeout>>>(
		new Map(),
	);
	const dislikeParity = useRef<Map<string, number>>(new Map());
	const dislikeTimers = useRef<Map<string, ReturnType<typeof setTimeout>>>(
		new Map(),
	);
	...
	const handleLike = (postId: string) => {
		...
		if (likeTimers.current.has(postId))
			clearTimeout(likeTimers.current.get(postId)!);
		likeTimers.current.set(
			postId,
			setTimeout(async () => {
	...
	const handleDislike = (postId: string) => {
		...
		if (dislikeTimers.current.has(postId))
			clearTimeout(dislikeTimers.current.get(postId)!);
		dislikeTimers.current.set(
			postId,
			setTimeout(async () => {
	...
	const handleHeart = (postId: string) => {
		...
		if (heartTimers.current.has(postId))
			clearTimeout(heartTimers.current.get(postId)!);
		heartTimers.current.set(
			postId,
			setTimeout(async () => {
				...
				try {
					...
					updatePost(postId, (p) => ({
						...p,
						heartsTotal: Math.max(0, (Number(p.heartsTotal) || 0) - count),
					}));
		...
		try {
			...
			updatePost(postId, (p) => ({
				...
				replies: p.replies.map((r) =>
					r.id === tempId
						? { ...reply, avatarUrl: reply.avatarUrl ?? options?.avatarUrl }
						: r,
				),
			}));
		} catch {
