...
function publish(next: Partial<ScrollNavState>) {
	...
	if (
		merged.footerHidden === state.footerHidden &&
...
function attach() {
	...
	const onScroll = (e: Event) => {
		...
		if (
			target === document ||
			(target instanceof HTMLElement && target.id === SCROLL_CONTAINER_ID)
		) {
...
export function useScrollNav() {
	...
	return {
		footerHidden: local.footerHidden,
		scrolled: local.scrolled,
		scrollToTop,
		scrollToBottom,
	};
}
