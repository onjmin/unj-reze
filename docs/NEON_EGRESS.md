...
### 4. 返信は常に「直近N件の窓」でしか読まない

件数の上限は `REPLIES_PAGE_SIZE`（20件、`lib/db/interface.ts`）ひとつに統一してある。

| 経路 | 返すもの |
|---|---|
| フィード `getPosts` | スレッドごとに新しい順で20件（ウィンドウ関数 `ROW_NUMBER`） |
| スレッド詳細 `getPost` | 同じく直近20件 |
| `GET /api/posts/[id]/replies` | 直近20件。`?before=<レス番号>` でその手前がもう1ページ |
| 専ブラ `.dat` | **例外**。`limit: RES_LIMIT` を明示してスレ全件 |

`.dat` だけが例外なのはプロトコルの都合で、専ブラは Range のバイトオフセットで
差分を取るため、途中から返すと既読位置が壊れる。

古いレスは**上へスクロールしたときだけ**追加で読む（`lib/hooks/useOlderReplies.ts`）。
ページングのカーソルは配列の添字ではなく**レス番号 `Post.num`**（OP=1、以降 `res.num`）。
窓で切り取った一覧では添字が実レス番と一致せず、`>>N` の採番もカーソルも壊れるため、
`num` はAPIのレスポンスに必ず載せる。

返信先も同様に **`Post.parentNum`（`res.parent_num`）を正**とする。`parentPostId` は窓の中で
`num → id` を解決した結果なので、親が窓の外にいると解決できずOPへフォールバックし、
掲示板モードの `>>N` 安価が消える。`>>N` をクリックしたときは、飛び先が未読込みなら
`loadOlderUntil()` が上限5ページまで遡ってから飛ぶ（無制限に遡ると全件取得と同じになる）。

返信本文を1行も使わない一覧は `getPosts(..., { withReplies: false })` で埋め込み自体を止める
（専ブラの `subject.txt`、`sitemap.ts`。300スレ×20返信を読んで捨てていた）。

副作用:
- フィードの「メディア」「返信」サブタブは、各スレッドの新しい20件からしか拾わない。
- 画面に出ている件数 ≠ スレの総レス数。総数は必ずサーバー由来の `repliesCount`
  （`threads.res_count - 1`。`COUNT(*)` は撃たない）を使う。窓の `replies.length` で
  数えると21件以上のスレが全部頭打ちになる。
...
## Workers の CPU 上限（10ms/req）

転送量とは別に、**Cloudflare Workers 無料枠は1リクエストあたり CPU 10ms** という硬い制限がある。
超えると `Error: Worker exceeded CPU time limit` で 500 になる（DB待ちなどのI/Oは含まない、純粋な計算時間）。

一度これで `/api/posts` と `/post/[id]` が全滅した。原因は **sqids のID変換**だった:

- `sqids.encode` は1回あたり約 **15µs**。
- `encodePost` は1投稿につき `id` / `threadId` / `parentPostId` / `gameId` を変換する。
- フィードは「スレッド＋返信」を全部encodeするので、420投稿で
  **ID変換だけで約17ms** に達していた（JSON化は 0.9ms なので、桁違いに重かった）。

**対処: sqids をやめて生の数値IDにした**（`lib/sqids.ts`）。

| 形状 | 投稿数 | sqids | 生ID |
|---|---|---|---|
| フィード 20スレ×5返信 | 120 | 5.9ms | 0.23ms |
| フィード 20スレ×20返信 | 420 | 17.8ms ❌ | 0.67ms |
| スレ詳細 500返信 | 501 | 14.6ms ❌ | 1.06ms |
| スレ詳細 2000返信 | 2001 | – | 4.01ms |

IDを隠す必要が無いのは、権限判定がすべて userId / slug / target_user の照合で行われており、
IDの推測困難性に依存していないため（投稿自体も公開フィードに出ている）。
`decodeId` は旧 sqids 形式も引き続き解釈するので、**既存の共有リンクは壊れない**。

引き換えに、IDが連番になるので「これまでの総投稿数」は外から分かる。

> それでも、1レスポンスに載せる投稿件数は転送量の観点で上限を持つこと（上の 4.）。
> CPU の余裕ができても Neon の 5GB/月は別枠。
