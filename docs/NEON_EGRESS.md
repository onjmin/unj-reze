...
`FEED_REPLIES_PER_THREAD`（**5件**）。ウィンドウ関数でスレッドごとに新しい順で絞る。
スレッド詳細（`getPost`）は従来どおり全件読む。

副作用: フィードの「メディア」「返信」サブタブは、各スレッドの新しい5件からしか拾わない。

**この値は転送量ではなく CPU で決まっている** — 下の「Workers の CPU 上限」を参照。
...
超えると `Error: Worker exceeded CPU time limit` で 500 になる（DB待ちなどのI/Oは含まれない、純粋な計算時間）。

実際にこれで `/api/posts` と `/post/[id]` が全滅した。犯人は **sqids のID変換**:

- `sqids.encode` は1回あたり約 **15µs**。
- `encodePost` は1投稿につき `id` / `threadId` / `parentPostId` / `gameId` を変換する。
- フィードは「スレッド＋返信」を全部encodeするので、20スレッド×20返信=420投稿で
  **ID変換だけで約17ms** に達し、JSON化(0.9ms)より遥かに重い。

対策は2つ入っている:

1. **`lib/sqids.ts` でメモ化**（`encodeId` / `decodeId`）。
   同じIDが何度も出てくる（`threadId` はスレッド内で共通、再リクエストでも同じ投稿が並ぶ）ため、
   アイソレートが温まっていれば実測で **13倍** 速い。純粋関数なので結果は変わらない。
2. **フィードの返信上限を 5 件に**。

実測（返信200文字・ローカルCPU）:

| 形状 | 投稿数 | 修正前 | 修正後(cold) | 修正後(warm) |
|---|---|---|---|---|
| フィード 20スレ×20返信 | 420 | 17.8ms ❌ | – | – |
| フィード 20スレ×5返信 | 120 | – | 5.9ms ✅ | 0.33ms |
| スレ詳細 500返信 | 501 | – | 14.6ms ❌ | 1.2ms |

> ⚠️ **未解決**: スレッド詳細は返信を全件読むため、**おおよそ300返信を超えると cold で10msを超える**。
> アイソレートが温まっていれば1.2msで通るので、症状は「たまに500」になる。
> 恒久対策はスレッド詳細の返信もページングすること（フィードと同じ `beforeId` 方式）。
> Workers 有料プラン（CPU 30s）へ上げれば、この制約自体が消える。

**新しくレスポンスへ投稿を増やすときは、必ず投稿件数 × 15µs を見積もること。**
